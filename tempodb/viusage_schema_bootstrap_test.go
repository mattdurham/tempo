...
// bootstrap-ordering fix (New()'s new blockpack.ApplyViUsageSchema call, tempodb.go). Proves
// New() alone, against a genuinely fresh Postgres instance with NO other schema-apply step,
// leaves viusage_entries in a state where blockpack.NewPgViUsageEntryStore(rw.pgPool) -- the
// exact construction ConfigureViUsage's registryFor (vi_usage_hook.go) and
// NewViBackfillDepsWithPgRegistry (vi_backfill.go) both use against this same pool -- can
// actually UpsertEntry without "relation \"viusage_entries\" does not exist".
...
func TestNew_AppliesViUsageSchema_PgViUsageEntryStoreUsableImmediately(t *testing.T) {
	...
	store := blockpack.NewPgViUsageEntryStore(rw.pgPool)
