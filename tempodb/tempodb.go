...
func New(cfg *Config, cacheProvider cache.Provider, logger gkLog.Logger) (Reader, Writer, Compactor, error) {
	...
	// rawRUncached/rawWUncached preserve the genuine, backend-specific RawReader/RawWriter
	// (e.g. *local.Backend, GCS's *readerWriter) from before the cache-wrap below. The VI/cube
	// registry Configure* calls further down use these, not the (possibly cache-wrapped) rawR/
	// rawW, because: (1) registry reads already opt out of caching via a nil CacheInfo at every
	// call site, so they never benefit from footer/bloom/column-idx/page caching in the first
	// place, and (2) backend_cache.readerWriter (below) implements only the base
	// backend.RawReader/RawWriter methods -- wrapping rawR/rawW in it silently hides
	// local.Backend's WriteAtomic and GCS's WriteVersioned/ReadVersioned from vblockpack's
	// optional-capability probes (rawobjectstore.go's atomicWriter, rawobjectstore_gcs.go's
	// isGCSBackend), which always fail against the wrapped type. Since cacheProvider is a hard
	// module dependency in the real app (cmd/tempo/app/modules.go), it is non-nil in every real
	// deployment, so this bypass is required for the registries' atomic/native conditional-write
	// paths to ever be reachable in production, not just in tests that pass a nil cacheProvider.
	rawRUncached, rawWUncached := rawR, rawW
	...
	if cfg.Block.Blockpack.ValueIndexEnabled {
		var s3cfg *s3.Config
		if cfg.Backend == backend.S3 {
			s3cfg = cfg.S3
		}
		vblockpack.ConfigureValueIndex(true, s3cfg, rawWUncached, cfg.Block.Blockpack.ValueIndexPrefix)
		// Wire cube ingest manager alongside value-index (same backend, same gate).
	...
	// Index-driven query path (blockpack issue #461). Always active on any backend
	// (2026-07-11 ruling widened this to be backend-agnostic, no opt-in) — the value
	// index lives in the same location as the trace blocks, whatever that backend is.
	// Falls back to a full block scan when the index lacks coverage for a query. See
	// the write-path gate above for why this omits a cfg.Block nil-check (provably
	// non-nil here already).
	...
	if cfg.Backend == backend.S3 {
		...
		vblockpack.ConfigureValueIndexQueryRaw(rawRUncached, viq.IndexPrefix, viq.CacheTTL, viq.ContentCacheBytes)
	}
	...
	// ConfigureValueIndexQueryRaw enables above). vu.DedicatedColumnsEnabled=false (R12)
	// makes ConfigureViUsage install a nil recorder, so every call site's
	// getViUsageRecorder()==nil check makes the hook a zero-function-call no-op -- the
	// full end-to-end loop (query -> usage hook -> registry -> trigger -> B2's backfill
	// launcher -> watermark persist -> B3's watermark cache -> back into the query path)
	// closes from this one call, on any backend.
	var s3cfg *s3.Config
	if cfg.Backend == backend.S3 {
		s3cfg = cfg.S3
	}
