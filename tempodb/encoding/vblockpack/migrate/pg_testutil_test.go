...
// modules/backendscheduler/filecatalog/pg_testutil_test.go (unexported test
// helpers cannot be imported across packages). Unlike either of those, this
// copy applies NO schema at all -- migrate.Apply itself is what this
// package's tests exercise, so the container must start with a genuinely
// empty database.
...
func newTestPostgresPool(t *testing.T) *pgxpool.Pool {
	...
	container, err := tcpostgres.Run(ctx, "postgres:16-alpine",
		...
		tcpostgres.WithUsername("migrate_test"),
		tcpostgres.WithPassword("migrate_test"),
