...
// (ExecuteMetricsTraceQL's decline contract).
//
// Search used to also have a SEPARATE plan-time decline story (#62's frontend decline,
// ErrPlanTimeLowSelectivityNoLimit, vcnt_fetch.go, which lived in the FRONTEND, not the
// querier) — that sentinel was REMOVED entirely by issue #535 (team-lead ruling, reversing R6):
// a query may only decline for a genuine index/cube coverage gap, never as a cost/selectivity
// heuristic for a query the system CAN answer correctly. ErrSearchNoCoverage itself is
// UNAFFECTED by that change — it is a real, querier-local coverage-gap decline (this specific
// block genuinely has no index coverage), not a selectivity heuristic, and remains exactly as
// it was.
//
// ErrSearchNoCoverage is returned by declineOutcomeBounded when a routine
...
// blockpack.SelectSearchStrategy still accepts frontend-side (F-6) — re-evaluated locally rather
// than threaded over the wire (no wire path from the frontend's QueryPlan.Strategy to this
// per-block call exists, and building one would have required protobuf generator surgery
// unavailable in this environment). This is the SAFE-DEFAULT backstop for declines
// boundedAuthorized doesn't cover (slice jobs already have their own ErrSliceIndexCoverageGap;
// declines on a query with no limit; a coarse/tenant-level CheckIndexCoverage signal that
// doesn't hold for this specific block) — converting the decline directly to a hard error rather
// than an implicit scan, matching #481's central anti-pattern elimination. Joins F-10's
// DeclineErrorToHTTPResponse mapper switch as a 4xx-class, actionable-message case.
