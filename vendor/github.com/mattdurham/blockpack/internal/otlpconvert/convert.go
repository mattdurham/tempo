...
// DecodeTraces decodes raw OTLP protobuf into a slice of TracesData entries.
func DecodeTraces(data []byte) ([]*tracev1.TracesData, error) {
	if len(data) == 0 {
		return nil, fmt.Errorf("empty input")
	}

	if traces, ok, err := decodeDelimitedExport(data); err != nil {
		return nil, err
	} else if ok {
		return traces, nil
	}

	if traces, ok, err := decodeDelimitedTracesData(data); err != nil {
		return nil, err
	} else if ok {
		return traces, nil
	}
	...
	if err := proto.Unmarshal(data, req); err == nil && len(req.ResourceSpans) > 0 {
		return []*tracev1.TracesData{{ResourceSpans: req.ResourceSpans}}, nil
	}

	td := &tracev1.TracesData{}
	if err := proto.Unmarshal(data, td); err == nil && len(td.ResourceSpans) > 0 {
		return []*tracev1.TracesData{td}, nil
	}

	return nil, fmt.Errorf("no OTLP traces found in input")
}

func decodeDelimitedExport(data []byte) ([]*tracev1.TracesData, bool, error) {
	reader := bufio.NewReader(bytes.NewReader(data))
	traces := make([]*tracev1.TracesData, 0)
	for {
		req := &collectortracev1.ExportTraceServiceRequest{}
		if err := protodelim.UnmarshalFrom(reader, req); err != nil {
			if err == io.EOF {
				break
			}
			if len(traces) == 0 {
		...
		traces = append(traces, &tracev1.TracesData{ResourceSpans: req.ResourceSpans})
	}
	if len(traces) == 0 {
		return nil, false, nil
	}
...
func decodeDelimitedTracesData(data []byte) ([]*tracev1.TracesData, bool, error) {
	reader := bufio.NewReader(bytes.NewReader(data))
	traces := make([]*tracev1.TracesData, 0)
	for {
		td := &tracev1.TracesData{}
		if err := protodelim.UnmarshalFrom(reader, td); err != nil {
			if err == io.EOF {
				break
			}
			if len(traces) == 0 {
...
func (b *traceBuilder) addSpan(resourceKey string, resource *resourcev1.Resource, resourceSchema string, scopeKey string, scope *commonv1.InstrumentationScope, scopeSchema string, span *tracev1.Span) {
