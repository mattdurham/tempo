...
func CompileTraceQL(query string) (*vm.Program, error) {
	...
	if _, isMetrics := result.(*traceqlparser.MetricsQuery); isMetrics {
	...
	if _, isStructural := result.(*traceqlparser.StructuralQuery); isStructural {
	...
	ast, ok := result.(*traceqlparser.FilterExpression)
	if !ok {
