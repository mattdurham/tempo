...
func TraceQLMetricsToSQL(metricsQuery *traceqlparser.MetricsQuery, startTime, endTime int64) (string, error) {
	if metricsQuery == nil {
...
func aggregateFuncToSQL(agg traceqlparser.AggregateFunc) (string, error) {
	switch agg.Name {
	case "rate":
...
func buildWhereClauses(filter *traceqlparser.FilterExpression, startTime, endTime int64) ([]string, error) {
	var clauses []string
