...
const (
	// IOCategoryUnknown indicates an unknown IO category.
	IOCategoryUnknown IOCategory = iota
	// IOCategoryFooter indicates footer data operations.
	IOCategoryFooter
	// IOCategoryMetadata indicates metadata operations.
	IOCategoryMetadata
	// IOCategoryBlockIndex indicates block index operations.
	IOCategoryBlockIndex
	// IOCategoryColumnIndex indicates column index operations.
	IOCategoryColumnIndex
	// IOCategoryDedicatedIndex indicates dedicated index operations.
	IOCategoryDedicatedIndex
	// IOCategoryBlockData indicates block data operations.
	IOCategoryBlockData
	// IOCategoryBloomFilter indicates bloom filter operations.
	IOCategoryBloomFilter
	// IOCategoryMetricStream indicates metric stream operations.
...
func NewDetailedTrackingReader(underlying ReaderProvider, logEnabled bool) (*DetailedTrackingReader, error) {
	if underlying == nil {
...
func NewDetailedTrackingReaderWithLatency(
	underlying ReaderProvider,
	latency time.Duration,
	logEnabled bool,
) (*DetailedTrackingReader, error) {
	if underlying == nil {
		return nil, fmt.Errorf("underlying provider cannot be nil")
	}
	return &DetailedTrackingReader{
...
func (d *DetailedTrackingReader) SetFileStructure(
	footerOffset, metadataOffset, metadataLen, metricStreamOffset, metricStreamLen int64,
	blockOffsets []uint64,
) {
	d.logMu.Lock()
	...
	for _, offset := range blockOffsets {
		d.blockOffsets[int64(offset)] = true //nolint:gosec
	}
}
...
// ReadAt reads bytes from the underlying data source at the specified offset with detailed tracking.
