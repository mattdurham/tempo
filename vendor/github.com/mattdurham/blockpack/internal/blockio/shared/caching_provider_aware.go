...
const (
	// RegionUnknown indicates an unknown cache region.
	RegionUnknown CacheRegion = iota
	// RegionFooter is the last 41 bytes - always cached.
	RegionFooter
	// RegionMetadata is schema and column info - always cached.
	RegionMetadata
	// RegionColumnData is column data - LRU with prefetching.
	RegionColumnData
	// RegionRowData is row data - adaptive based on access pattern.
	RegionRowData
)
...
func NewDataAwareCachingProvider(
	underlying ReaderProvider,
	config CachingProviderConfig,
) (*DataAwareCachingProvider, error) {
	if underlying == nil {
	...
	if err != nil {
		return nil, fmt.Errorf("create caching reader provider: %w", err)
	}
...
func (d *DataAwareCachingProvider) readWithLRUCache(
	p []byte,
	off int64,
	region CacheRegion,
	dataType DataType,
) (int, error) {
	cacheKey := d.makeCacheKey(off, len(p))
...
func (d *DataAwareCachingProvider) readWithPrefetch(
	p []byte,
	off int64,
	region CacheRegion,
	dataType DataType,
