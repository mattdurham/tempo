...
func (cb *columnBuilder) buildUint64Data(buf *bytes.Buffer, spanCount, presentCount int, useSparse bool, presenceRLE []byte, encoder *zstd.Encoder) error {
	...
	for i := range spanCount {
		if isBitSet(cb.present, i) {
			...
			if !ok {
				dictIdx = uint32(len(uintDictVals))
