...
func (cb *columnBuilder) setBytes(idx int, value []byte) error {
	...
	if cb.useXOR || cb.usePrefix {
		...
		if !ok {
			dictIdx = uint32(len(cb.bytesDictVals)) //nolint:gosec
...
func (cb *columnBuilder) buildBytesData(
	buf *bytes.Buffer,
	...
	if cb.useXOR {
		return encodings.BuildXORBytes(
			encoder,
			buf,
			spanCount,
			presentCount,
	...
	if cb.usePrefix {
		return encodings.BuildPrefixBytes(
			encoder,
			buf,
			spanCount,
			presentCount,
	...
	_ = binary.Write(&dictBuf, binary.LittleEndian, uint32(len(cb.bytesDictVals))) //nolint:gosec
	for _, val := range cb.bytesDictVals {
		_ = binary.Write(&dictBuf, binary.LittleEndian, uint32(len(val))) //nolint:gosec
		if len(val) > 0 {
	...
	if cb.name == columnNameTraceID {
		return encodings.BuildBytesDictionaryDelta(
			encoder,
			buf,
			spanCount,
			presentCount,
	...
	encodingKind, err := encodings.ChooseBytesEncodingKind(
		spanCount,
		presentCount,
		useSparse,
		presenceRLE,
	...
	if encodingKind == encodingKindInlineBytes || encodingKind == encodingKindSparseInlineBytes {
		return encodings.BuildBytesInline(
			buf,
			spanCount,
			presentCount,
			useSparse,
	...
	return encodings.BuildBytesDictionary(
		encoder,
		buf,
		spanCount,
		presentCount,
