...
//
// The encoder parameter is passed to column builders that need compression (e.g., bytes dictionaries).
// Using a per-Writer encoder instead of sync.Pool prevents memory accumulation - pools retain
// all encoders until GC runs, causing OOM with large conversions (128 MB per encoder × hundreds of blocks).
func (cb *columnBuilder) buildData(spanCount int, encoder *zstd.Encoder) ([]byte, columnStatsBuilder, error) {
	if cb == nil {
		return nil, columnStatsBuilder{}, fmt.Errorf("nil column builder")
	}
	if cb.length() != spanCount {
		return nil, columnStatsBuilder{}, fmt.Errorf(
			"column %s length mismatch: %d != %d",
			cb.name,
			cb.length(),
			spanCount,
		)
	...
	switch cb.typ {
	case ColumnTypeString:
		err = cb.buildStringData(buf, spanCount, presentCount, useSparse, presenceRLE, encoder)
	case ColumnTypeInt64:
		err = cb.buildInt64Data(buf, spanCount, presentCount, useSparse, presenceRLE, encoder)
	case ColumnTypeUint64:
		err = cb.buildUint64Data(buf, spanCount, presentCount, useSparse, presenceRLE, encoder)
	case ColumnTypeBool:
		err = cb.buildBoolData(buf, spanCount, presentCount, useSparse, presenceRLE, encoder)
	case ColumnTypeFloat64:
		err = cb.buildFloat64Data(buf, spanCount, presentCount, useSparse, presenceRLE, encoder)
	case ColumnTypeBytes:
		err = cb.buildBytesData(buf, spanCount, presentCount, useSparse, presenceRLE, encoder)
	default:
