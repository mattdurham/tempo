...
var (
	// Reuse zstd decoder for better performance
...
func parseColumnReuse(name string, typ ColumnType, expectedRows int, data []byte, statsData []byte, reusable *Column, a *arena.Arena) (*Column, error) {
...
func decodeDeltaUint64Column(name string, rd *sliceReader, statsData []byte, col *Column, a *arena.Arena) (*Column, error) {
	...
	presence, err := decodePresenceRLE(presenceRLE, int(spanCount))
	if err != nil {
	...
	for i := 0; i < int(spanCount); i++ {
		if isBitSet(presence, i) {
			// Read offset based on width
			var offset uint64
			switch width {
			case 1:
				b, err := offsetReader.readUint8()
				if err != nil {
					return nil, fmt.Errorf("column %s: %w", name, err)
				}
				offset = uint64(b)
			case 2:
				w, err := offsetReader.readUint16()
				if err != nil {
					return nil, fmt.Errorf("column %s: %w", name, err)
				}
				offset = uint64(w)
			case 4:
				d, err := offsetReader.readUint32()
				if err != nil {
					return nil, fmt.Errorf("column %s: %w", name, err)
				}
				offset = uint64(d)
...
func decodeXORBytesColumn(name string, rd *sliceReader, statsData []byte, col *Column, a *arena.Arena) (*Column, error) {
	spanCount, err := rd.readUint32()
	if err != nil {
		return nil, fmt.Errorf("column %s: %w", name, err)
	}
	...
	presence, err := decodePresenceRLE(presenceData, int(spanCount))
	if err != nil {
		return nil, fmt.Errorf("column %s: %w", name, err)
	}
...
func decodePrefixBytesColumn(name string, rd *sliceReader, statsData []byte, col *Column, a *arena.Arena) (*Column, error) {
	spanCount, err := rd.readUint32()
	if err != nil {
		return nil, fmt.Errorf("column %s: %w", name, err)
	}
...
func decodeDictionaryColumn(name string, typ ColumnType, expectedRows int, rd *sliceReader, statsData []byte, col *Column, flags columnEncodingFlags, a *arena.Arena) (*Column, error) {
	switch typ {
...
func decodeStringDictionaryColumn(name string, expectedRows int, rd *sliceReader, statsData []byte, col *Column, flags columnEncodingFlags, a *arena.Arena) (*Column, error) {
	...
	dictCount, err := dictReader.readUint32()
	if err != nil {
		return nil, fmt.Errorf("column %s: %w", name, err)
	}
	...
	dict := make([]string, int(dictCount))
	for i := 0; i < int(dictCount); i++ {
		strLen, err := dictReader.readUint32()
		if err != nil {
			return nil, fmt.Errorf("column %s: %w", name, err)
		}
		strBytes, err := dictReader.readBytes(int(strLen))
		if err != nil {
			return nil, fmt.Errorf("column %s: %w", name, err)
		}
...
func decodeInt64DictionaryColumn(name string, expectedRows int, rd *sliceReader, statsData []byte, col *Column, flags columnEncodingFlags, a *arena.Arena) (*Column, error) {
	width, err := readIndexWidth(rd, name)
	if err != nil {
		return nil, err
	}
	...
	// CRITICAL: Dictionary must use heap allocation, NOT arena allocation.
	// The dictionary is stored in the Column and must outlive the arena.
	dict := make([]int64, int(dictCount))
	for i := 0; i < int(dictCount); i++ {
		val, err := dictReader.readInt64()
		if err != nil {
			return nil, fmt.Errorf("column %s: %w", name, err)
		}
		dict[i] = val
...
func decodeUint64DictionaryColumn(name string, expectedRows int, rd *sliceReader, statsData []byte, col *Column, flags columnEncodingFlags, a *arena.Arena) (*Column, error) {
	width, err := readIndexWidth(rd, name)
	if err != nil {
		return nil, err
	}
	...
	// CRITICAL: Dictionary must use heap allocation, NOT arena allocation.
	// The dictionary is stored in the Column and must outlive the arena.
	dict := make([]uint64, int(dictCount))
	for i := 0; i < int(dictCount); i++ {
		val, err := dictReader.readUint64()
		if err != nil {
			return nil, fmt.Errorf("column %s: %w", name, err)
		}
		dict[i] = val
...
func decodeBoolDictionaryColumn(name string, expectedRows int, rd *sliceReader, statsData []byte, col *Column, flags columnEncodingFlags, a *arena.Arena) (*Column, error) {
	width, err := readIndexWidth(rd, name)
	if err != nil {
		return nil, err
	}
	...
	// CRITICAL: Dictionary must use heap allocation, NOT arena allocation.
	// The dictionary is stored in the Column and must outlive the arena.
	dict := make([]uint8, int(dictCount))
	for i := 0; i < int(dictCount); i++ {
		val, err := dictReader.readUint8()
		if err != nil {
			return nil, fmt.Errorf("column %s: %w", name, err)
		}
		dict[i] = val
...
func decodeFloat64DictionaryColumn(name string, expectedRows int, rd *sliceReader, statsData []byte, col *Column, flags columnEncodingFlags, a *arena.Arena) (*Column, error) {
	width, err := readIndexWidth(rd, name)
	if err != nil {
		return nil, err
	}
	...
	// CRITICAL: Dictionary must use heap allocation, NOT arena allocation.
	// The dictionary is stored in the Column and must outlive the arena.
	dict := make([]float64, int(dictCount))
	for i := 0; i < int(dictCount); i++ {
		val, err := dictReader.readFloat64()
		if err != nil {
			return nil, fmt.Errorf("column %s: %w", name, err)
		}
		dict[i] = val
...
func decodeInlineBytesColumn(name string, expectedRows int, rd *sliceReader, statsData []byte, col *Column, isSparse bool, a *arena.Arena) (*Column, error) {
	rowCount, present, err := readRowCountAndPresence(rd, name)
	if err != nil {
		return nil, err
	}
	...
	if isSparse {
		...
		if readCount != int(presentCount) {
			return nil, fmt.Errorf("column %s: sparse inline count mismatch: read %d expected %d", name, readCount, presentCount)
		}
	} else {
		for i := 0; i < int(rowCount); i++ {
			l, err := rd.readUint32()
...
func decodeBytesDictionaryColumn(name string, expectedRows int, rd *sliceReader, statsData []byte, col *Column, flags columnEncodingFlags, a *arena.Arena) (*Column, error) {
	width, err := readIndexWidth(rd, name)
	if err != nil {
		return nil, err
	}
	...
	// CRITICAL: Dictionary must use heap allocation, NOT arena allocation.
	// The dictionary is stored in the Column and must outlive the arena.
	dict := make([][]byte, int(dictCount))
	for i := 0; i < int(dictCount); i++ {
		length, err := dictReader.readUint32()
		if err != nil {
			return nil, fmt.Errorf("column %s: %w", name, err)
		}
		valBytes, err := dictReader.readBytes(int(length))
		if err != nil {
			return nil, fmt.Errorf("column %s: %w", name, err)
		}
...
func readCompressedDict(rd *sliceReader, name string) ([]byte, error) {
	...
	if int(dictLen) > rd.remaining() {
		return nil, fmt.Errorf("column %s: compressed dictionary length %d exceeds remaining %d", name, dictLen, rd.remaining())
	}
...
func readIndexValues(rd *sliceReader, name string, width uint8, rowCount uint32, present []byte, isRLE bool, isSparse bool, values []uint32) error {
	if isRLE {
		...
		decodedIndexes, err := decodeIndexRLE(rleBytes, int(indexCount))
		if err != nil {
	...
	if isSparse {
		...
		if readCount != int(presentCount) {
			return fmt.Errorf("column %s: sparse index count mismatch: read %d expected %d", name, readCount, presentCount)
		}
		return nil
	}
...
func decodeDeltaDictionaryBytesColumn(name string, rd *sliceReader, statsData []byte, col *Column, flags columnEncodingFlags, a *arena.Arena) (*Column, error) {
