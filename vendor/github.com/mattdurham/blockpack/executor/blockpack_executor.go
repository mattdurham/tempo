...
func isIntrinsicColumn(name string) bool {
	switch name {
	case "trace:id", "span:id", "span:parent_id", "span:name",
		"span:start", "span:end", "span:duration",
		"span:status", "span:kind", "span:status_message",
		"resource.service.name": // Resource attributes use dots, not colons
		return true
	}
	return false
}
...
func extractResultColumns(program *vm.Program, opts QueryOptions) map[string]struct{} {
	...
	intrinsics := []string{
		"trace:id", "span:id", "span:parent_id", "span:name",
		"span:start", "span:end", "span:duration",
		"span:status", "span:kind", "span:status_message",
	}
...
func blockMightMatchValuePredicates(block *blockpackio.Block, predicates *QueryPredicates) bool {
	// For OR queries, use UNION logic (any predicate can match).
	// For AND queries, use INTERSECTION logic (all predicates must match).
	// Note: This function is only called when !predicates.HasOROperations (line 578),
	// so we should always use intersection logic here.
	useUnion := predicates.HasOROperations

	if useUnion {
		// Union logic: block matches if ANY column matches
		for columnName, values := range predicates.AttributeEquals {
			...
			for _, possibleColumn := range possibleColumns {
				...
				if col == nil {
					continue // This column doesn't exist, check next one
				}
				if !col.Stats.HasValues {
					continue // This column has no values, check next one
				}
				matches := valueMinMaxAnyMatch(col, values)
				if matches {
					return true // Found a matching column
				}
			}
		}
		return false // None of the columns matched
	}

	// Intersection logic: block must match ALL columns
	for columnName, values := range predicates.AttributeEquals {
		// For unscoped attributes, check all possible column names
		possibleColumns := GetPossibleColumnNames(columnName)
		found := false
