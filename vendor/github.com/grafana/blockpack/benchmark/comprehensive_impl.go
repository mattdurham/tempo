...
// Extracted trace IDs for trace-by-id queries
...
//
//nolint:deadcode // Public utility for external benchmark scripts
func ExecuteBlockpackQuery(b *testing.B, blockpackPath, traceql string) *QueryResult {
	b.Helper()
	start := time.Now()
	...
	matches, _, err := blockpack.QueryTraceQL(context.Background(), r, traceql, blockpack.QueryOptions{})
	if err != nil {
...
//
//nolint:deadcode // Public utility for external benchmark scripts
func ExecuteParquetQuery(b *testing.B, parquetPath, traceql string) *QueryResult {
	b.Helper()
