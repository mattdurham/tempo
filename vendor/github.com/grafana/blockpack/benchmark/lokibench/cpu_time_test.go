...
// GC time from user time. Its coarser resolution only affects the GC column.
//
...
func readCPUSecs() cpuSnapshot {
	...
	s2 := []metrics.Sample{{Name: "/cpu/classes/user:cpu-seconds"}}
	metrics.Read(s2)
	if s2[0].Value.Kind() == metrics.KindFloat64 {
		return cpuSnapshot{user: s2[0].Value.Float64(), gc: gcSecs}
	}
