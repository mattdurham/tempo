...
// --- VECTOR() compiler tests ---

func TestCompileVectorAI_NilEmbedderReturnsEmptyPredicate(t *testing.T) {
	// VECTOR_AI with nil embedder compiles to a no-match predicate (LIVESTORE compat).
	fe := mustParseFilter(t, `{ VECTOR_AI("test query") }`)
	prog, err := CompileTraceQLFilterWithOptions(fe, CompileOptions{Embedder: nil})
	assert.NoError(t, err, "VECTOR_AI with nil embedder should not error")
	assert.True(t, prog.HasVector)
	assert.Nil(t, prog.QueryVector, "no embedder → no query vector")
}

func TestCompileVectorAll_NilEmbedderReturnsEmptyPredicate(t *testing.T) {
	fe := mustParseFilter(t, `{ VECTOR_ALL("search query") }`)
	prog, err := CompileTraceQLFilterWithOptions(fe, CompileOptions{Embedder: nil})
	assert.NoError(t, err, "VECTOR_ALL with nil embedder should not error (LIVESTORE compat)")
	assert.True(t, prog.HasVector)
	assert.True(t, prog.VectorAll)
	assert.Nil(t, prog.QueryVector, "no embedder → no query vector")
}

func TestCompileVectorExpr_EmbeddingColumnInPredicates(t *testing.T) {
	// Parse VECTOR() — we expect the embedding column to appear in extracted predicates.
	fe := mustParseFilter(t, `{ VECTOR("slow database queries") }`)
	// We cannot call Embed without a real model, so verify extractTraceQLPredicates works.
	preds := extractTraceQLPredicates(fe.Expr)
	require.NotNil(t, preds)
	found := false
	for _, n := range preds.Nodes {
		if n.Column == embeddingColumnName {
			found = true
			assert.Equal(t, DefaultVectorThreshold, n.VectorThreshold)
		}
	}
	assert.True(t, found, "embedding column must appear in extracted predicates for VECTOR()")
}

func TestCompileVectorExpr_EmbeddingColumnInPredicatesColumns(t *testing.T) {
	fe := mustParseFilter(t, `{ VECTOR("errors") }`)
	preds := extractTraceQLPredicates(fe.Expr)
	require.NotNil(t, preds)
	found := false
	for _, col := range preds.Columns {
		if col == embeddingColumnName {
			found = true
		}
	}
	assert.True(t, found, "embedding column must appear in Predicates.Columns")
}
