...
// tsIndexEntry is a test-only struct describing one timestamp index entry.
// Production code stores entries as raw bytes (see NOTE-PERF-TS in ts_index.go).
...
// encodeTSEntries encodes entries into the 20-byte-per-entry wire format
// (minTS[8 LE] + maxTS[8 LE] + blockID[4 LE]) used by BlocksInTimeRange.
func encodeTSEntries(entries []tsIndexEntry) []byte {
	buf := make([]byte, len(entries)*20)
	for i, e := range entries {
		binary.LittleEndian.PutUint64(buf[i*20:], e.minTS)
		binary.LittleEndian.PutUint64(buf[i*20+8:], e.maxTS)
		binary.LittleEndian.PutUint32(buf[i*20+16:], e.blockID)
	}
	return buf
}
...
func TestBlocksInTimeRange_AllMatch(t *testing.T) {
	...
	r := &Reader{tsRaw: encodeTSEntries(entries), tsCount: len(entries)}
