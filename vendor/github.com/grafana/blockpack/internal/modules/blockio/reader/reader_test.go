...
func TestLeanReader_ThreeIO(t *testing.T) {
	...
	assert.NotEmpty(t, entries, "lean reader must return trace entries via compact index")
}
...
func TestDecompressionBombGuard(t *testing.T) {
	...
	addSpanToWriter(t, w, traceID, span, map[string]any{"service.name": "bomb-svc"})
	_, err := w.Flush()
	require.NoError(t, err)
	...
	// Compact index starts immediately after the file header (22 bytes).
	compactStart := headerOffset + 22
	...
	// decompression guard for older-format files).
	...
	// The reader must reject this file with an error mentioning "snappy decoded size".
...
func TestReader_FooterVersionMismatch(t *testing.T) {
	...
	// The writer writes a V4 footer (34 bytes at the end of the file).
	// Corrupt the V4 version bytes (fileSize-34) AND the V3 version bytes (fileSize-22)
	// so that both footer-format detections fail.
