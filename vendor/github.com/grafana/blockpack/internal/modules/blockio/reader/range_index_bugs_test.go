...
// TestCompareRangeKey_Uint64_Ordering verifies the ordering branches in
// compareRangeKey for ColumnTypeRangeUint64 (range_index.go:304,307).
func TestCompareRangeKey_Uint64_Ordering(t *testing.T) {
	small := encodeUint64Key(1)
	large := encodeUint64Key(100)

	// a < b → returns -1 (covers range_index.go:304.14,306.4).
	assert.Equal(t, -1, reader.CompareRangeKeyExported(shared.ColumnTypeRangeUint64, small, large),
		"compareRangeKey must return -1 when a < b for uint64")

	// a > b → returns 1 (covers range_index.go:307.14,309.4).
	assert.Equal(t, 1, reader.CompareRangeKeyExported(shared.ColumnTypeRangeUint64, large, small),
		"compareRangeKey must return 1 when a > b for uint64")

	// a == b → returns 0.
	assert.Equal(t, 0, reader.CompareRangeKeyExported(shared.ColumnTypeRangeUint64, small, small),
		"compareRangeKey must return 0 when a == b for uint64")
}
