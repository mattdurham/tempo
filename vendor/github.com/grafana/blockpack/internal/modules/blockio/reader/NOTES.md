...
## NOTE-144: Concurrent Intrinsic-Column Prefetch for the I/O-Bound Metrics Fast Path (2026-06-09)

The trace-metrics intrinsic fast path (executor `executeTraceMetricsIntrinsic`) reads ~4
intrinsic columns per block — span:start, the predicate column(s), the group-by column, and the
aggregate field — through repeated `GetIntrinsicColumn` / `GetIntrinsicColumnBlob` calls. Each
cache miss is exactly one ranged object-storage GET (`readRange` → one `provider.ReadAt`), and the
calls run strictly serially within a block. At object-storage latencies (50–100 ms/GET) and an
observed ~44% intrinsic-section cache hit rate, this is 150–300 ms of serial I/O wait per block
before any work — the dominant cost on the I/O-latency-bound metrics path (Pyroscope: querier at
~33% CPU, 90% of block delay in selectgo waiting for data). The prior eight iterations
(NOTE-133..143) optimized only the CPU scan and so could not move M8's wall time.

`PrefetchIntrinsicColumns(names)` collapses that serial chain into a single concurrent round.
Only the independent `readRange` I/O and decode run in worker goroutines (bounded by an
`errgroup` with `SetLimit(min(len(jobs), 8))`); each worker writes only its own local result.
Writes to the per-Reader `intrinsicDecoded` map are serialized under `intrinsicMu` via
`storeDecodedIntrinsic`, exactly as `GetIntrinsicColumn` does, and the read-only `intrinsicIndex`
TOC map is snapshotted on the calling goroutine before fan-out — so the Reader's "NOT safe for
concurrent use" TOC map is never mutated concurrently. `parsedIntrinsicCache` and
`r.cache.GetOrFetchIntrinsic` are already process-safe (shared across Readers/blocks today).

The method is advisory and best-effort: columns absent from the TOC, span:end (synthesized from
span:start + span:duration, both of which the prefetch warms), already-decoded columns, and any
per-column read/decode error are silently skipped, and no error is returned. After it runs, the
subsequent serial calls hit the warm per-Reader/process/blob cache and issue no further I/O.
Because `GetOrFetchIntrinsic` dedups, the GET count and bytes transferred are unchanged versus the
lazy path — only their serialization differs, so the object-storage I/O invariant (one read per
column, no new per-column reads) holds. On total prefetch failure the lazy serial path runs
unchanged, so results are byte-identical — the optimization can only remove latency, never change
behavior.

This mirrors the W-concurrent-I/O-for-latency-hiding philosophy (NOTE-058 in the executor) applied
to the intrinsic metrics path rather than the block pipeline. No file-format change.

Back-ref: `internal/modules/blockio/reader/intrinsic_reader.go:PrefetchIntrinsicColumns`,
`internal/modules/blockio/reader/intrinsic_reader.go:storeDecodedIntrinsic`
Caller: `internal/modules/executor/metrics_trace_intrinsic.go:executeTraceMetricsIntrinsic`
Test: `internal/modules/blockio/reader/intrinsic_reader_test.go:TestPrefetchIntrinsicColumns`
