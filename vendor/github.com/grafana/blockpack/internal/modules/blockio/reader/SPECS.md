...
## SPEC-IC-1: GetIntrinsicColumn — Returns nil Without Error When No Section
*Added: 2026-03-11*

`GetIntrinsicColumn(name string)` returns `(nil, nil)` (no error) when:
- The file has no intrinsic section (v3 footer, or v4 footer with `intrinsicIndexLen == 0`).
- The named column is not in the TOC (e.g., a dynamic attribute column name).

It only returns a non-nil error when an I/O or decoding error occurs while reading/decoding
an existing column blob.

Back-ref: `internal/modules/blockio/reader/intrinsic_reader.go:GetIntrinsicColumn`

---

## SPEC-IC-2: GetIntrinsicColumn — Single I/O, Lazy Decode
*Added: 2026-03-11*

The first call to `GetIntrinsicColumn(name)` performs exactly one `readRange` I/O to fetch
the column blob. Subsequent calls return the cached `*shared.IntrinsicColumn` result without
any additional I/O or re-decoding.

The TOC itself is eagerly parsed once during `NewReaderFromProvider` (one additional `readRange`
call if `intrinsicIndexLen > 0`). Individual column blobs are decoded on first access only.

**Thread safety:** `GetIntrinsicColumn` is NOT safe for concurrent use without external
synchronization (it mutates `intrinsicDecoded`).
...
## SPEC-IC-3: HasIntrinsicSection — Reports TOC Presence
*Added: 2026-03-11*

`HasIntrinsicSection()` returns `true` if and only if the TOC was successfully parsed and
contains at least one column entry (`len(r.intrinsicIndex) > 0`).

Returns `false` for:
- v3 footer files (no intrinsic section in the format).
- v4 footer files where `intrinsicIndexLen == 0` (writer wrote empty section, e.g., due to
  `overCap()` being true).
- Files where TOC parsing failed (error returned from `NewReaderFromProvider`).

Back-ref: `internal/modules/blockio/reader/intrinsic_reader.go:HasIntrinsicSection`
