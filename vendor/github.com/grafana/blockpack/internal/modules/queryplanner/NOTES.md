...
> **PARTIALLY SUPERSEDED 2026-06-29 (executor NOTE-477):** `pruneByColStats` was removed when
> in-file block pruning was dropped (value index now owns pruning). `Plan.PrunedByColStats` and
> its `blockpack.planner.pruned_by_colstats` span attribute remain for wire/telemetry stability
> but are now always 0. Safe to retire in a later cleanup.
