...
## NOTE-VI-095 — `ParseFilename`/`ParseFilenameV2` must reject any name not actually ending in `.blockpack`, not silently accept it (live data-loss incident hotfix, root cause 2 of 2)

Date: 2026-07-10

**The bug.** Both `ParseFilename` and `ParseFilenameV2` computed `base := strings.TrimSuffix(name, ".blockpack")` with no check that the suffix was actually present. `strings.TrimSuffix` silently no-ops and returns its input unchanged when the suffix doesn't match, so any filename NOT ending in `.blockpack` could still "successfully" parse as long as its dash-structure happened to fit `L<level>-<id>` (v1) or `L<level>-<minSec>-<maxSec>-<id>` (v2). Confirmed live in a dev Kubernetes environment: a VCNT (valuecounts) `.vcnt` file with a VI-v2-shaped name (e.g. `L0-100-200-abc.vcnt`) parsed successfully via `ParseFilenameV2`, which combined with root cause 1 (NOTE-VI-096, valueindexcompactor's work-list builder not excluding VCNT's `unique_values/` directory) let VI's compactor treat a legitimate VCNT data file as one of its own candidates. VI's compactor then did a magic-byte `Peek()` (VI's magic is `VBG2`, VCNT's is `VCN1`), saw a mismatch, concluded the file was legacy junk, and called `Delete()` on it — permanently destroying legitimate VCNT data.

**The fix.** Both functions now check `if base == name { return ..., fmt.Errorf(...missing .blockpack suffix...) }` immediately after the `TrimSuffix` call, before any further parsing — mirroring `internal/modules/valuecounts/filename.go`'s `ParseFilename`/`ParseFilenameV2`, which already had this check for the `.vcnt` suffix. This is defense in depth: even if a `.vcnt` file is ever listed anywhere VI code scans (e.g. a future directory-layout change reintroduces the class of bug NOTE-VI-096 fixes), it can no longer parse as a valid VI filename.

**Why this is independent from NOTE-VI-096 and both had to be fixed.** Root cause 1 (the compactor listing `unique_values/` as a candidate column directory) and root cause 2 (this note) are two separate failure paths into the same outcome — fixing only one leaves the other as a live path to the identical data loss. Root cause 1 is the directory-scope bug that let a `.vcnt` file be discovered by VI's compactor at all; root cause 2 is the filename-parsing bug that let it dash-parse as a plausible VI candidate once discovered. Either fix alone closes one path but not the other.
