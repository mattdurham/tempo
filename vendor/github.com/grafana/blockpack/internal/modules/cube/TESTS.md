...
| TEST-CUBE-138 | `TestRunCubeBackfill_ZeroDimensionEntryBackfillsSuccessfully` (renamed 2026-07-16, issue #511; was `TestRunCubeBackfill_ZeroDimensionEntryDeclinesImmediately`) | cube_backfill_runner_test.go (root package) | A `Dimensions: nil` entry now backfills successfully (`err == nil`) via `processMinuteZeroDim`'s DurationColumn-anchor strategy — assertion inverted from the prior decline-and-burn-zero-retries expectation; see SPEC-CUBE-033's Addendum and NOTE-CUBE-035 |
...
## TEST-CUBE-140..146 — Zero-dimension backfill via DurationColumn anchor (issue #511)

| ID | Test | File | Scenario/Setup/Assertions |
|----|------|------|---------------------------|
| TEST-CUBE-140 | `TestBackfiller_ZeroDim_WritesOneCellPerMinute` | backfill_test.go | A zero-dimension `RegistryEntry` (`Dimensions: nil`) with real VI spans present in a minute window: `processMinuteZeroDim` writes exactly one `(AllDimSentinel, AllDimSentinel)` cell per minute, `Count` matching the number of anchor (`DurationColumn`) entries in that window |
| TEST-CUBE-141 | `TestBackfiller_ZeroDim_EmptyMinuteWritesNothing` | backfill_test.go | No VI data for the minute window: `processMinuteZeroDim` returns `(nil)` and writes no file — the sparse-cube contract holds for the zero-dim path exactly as it does for the dimensioned path |
| TEST-CUBE-142 | `TestBackfiller_ZeroDim_PopulatesAggAttrFromAnchorSourceRef` | backfill_test.go | `processMinuteZeroDim` sets `vals[DurationColumn]` directly from the anchor entry's own `SourceRef` (no separate join/lookup for `DurationColumn`); the resulting cell's `Aggs` duration `SampleCount`/`Sum` reflect the anchor value exactly |
| TEST-CUBE-143 | `TestBackfiller_ZeroDim_MultiAggAttrJoinsSeparateColumnCorrectly` | backfill_test.go | A zero-dim entry with `AggAttrs` beyond `duration` (e.g. a second numeric column): `lookupAggAttrValues` correctly joins the extra column's VI entries back to each anchor span by `(TraceID, SpanID)`, populating that attribute's `Aggs` record without cross-contaminating `DurationColumn`'s own anchor-sourced value |
| TEST-CUBE-144 | `TestBackfiller_ZeroDim_DoesNotDoubleLookupDurationColumn` | backfill_test.go | Asserts `lookupAggAttrValues` is called with `extraExcluded == map[string]bool{DurationColumn: true}` from `processMinuteZeroDim`, and that only ONE `LookupColumn(DurationColumn, ...)` call occurs for the minute (the anchor lookup) — confirms the deliberate anchor-reuse-avoids-double-fetch efficiency choice (`NOTE-CUBE-035`), not merely that the output is correct |
| TEST-CUBE-145 | `TestBackfiller_ZeroDim_AppliesRegisteredFilter` | backfill_test.go | A zero-dim entry with a registered `ColumnFilter` (e.g. a numeric threshold): `processMinuteZeroDim` routes every anchor span through the accumulator's normal `Add`/filter path exactly like the dimensioned path — a span failing the filter is not counted, mirroring `SPEC-CUBE-027`'s single-source-of-truth `ColumnFilterToFilter` conversion |
| TEST-CUBE-146 | `TestBackfill_ZeroDimSentinelMatchesForwardIngest` | cube_zerodim_publicapi_test.go (root package) | Spec-invariant regression test: builds one cube file via the forward-ingest-style `Definition{Dim1Column: AllDimSentinel, Dim2Column: AllDimSentinel}` path (`CubeRegistryEntryToDefinition`) and one via the real `Backfiller`/`processMinuteZeroDim`/`RunCubeBackfill` path for the SAME zero-dimension `RegistryEntry`, feeds both into `Rollup`, and asserts exactly ONE merged series results — the zero-dimension analog of TEST-CUBE-139's single-dimension sentinel-match test; confirms forward-ingest and backfill agree on `AllDimSentinel` for the zero-dim case too, or `CubeRollup` would silently produce two unmerged series |

**Next free ID: TEST-CUBE-147.**
