...
// --- Vector centroid pruning tests ---

func TestFindVectorThreshold_NilPreds_ReturnsDefault(t *testing.T) {
	got := findVectorThreshold(nil)
	assert.Equal(t, vm.DefaultVectorThreshold, got)
}

func TestFindVectorThreshold_NodeWithThreshold(t *testing.T) {
	preds := &vm.QueryPredicates{
		Nodes: []vm.RangeNode{
			{
				Column:          "__embedding__",
				QueryVector:     []float32{1, 0},
				VectorThreshold: 0.5,
			},
		},
	}
	got := findVectorThreshold(preds)
	assert.Equal(t, float32(0.5), got)
}

func TestFindVectorThreshold_NoVectorNode_ReturnsDefault(t *testing.T) {
	preds := &vm.QueryPredicates{
		Nodes: []vm.RangeNode{
			{Column: "resource.service.name", Values: []vm.Value{{Type: vm.TypeString, Data: "svc"}}},
		},
	}
	got := findVectorThreshold(preds)
	assert.Equal(t, vm.DefaultVectorThreshold, got)
...
func TestFileLevelVectorPrune_NoVectorIndex_KeepsAllBlocks(t *testing.T) {
	// Build a writer without VectorDimension — no vector index in the file.
	...
	require.NoError(t, w.AddSpan(
		...
		map[string]any{"service.name": "svc"}, "", nil, "",
	))
	_, err = w.Flush()
	require.NoError(t, err)
	...
	program := &vm.Program{
		...
		Predicates: &vm.QueryPredicates{
			Nodes: []vm.RangeNode{
				{Column: "__embedding__", QueryVector: []float32{1, 0, 0}, VectorThreshold: 0.3},
			},
		},
	}
	selected := []int{0}
	result := fileLevelVectorPrune(r, program, selected)
	// No vector index → no pruning → all blocks kept.
	assert.Equal(t, selected, result, "no vector index should keep all blocks")
}

// buildVectorFile writes spans with embeddings and returns a Reader over the resulting file.
// dim sets the VectorDimension; each vecDef defines a span with its embedding vector.
func buildVectorFile(t *testing.T, dim int, maxBlockSpans int, defs []vecSpanDef) *modules_reader.Reader {
	t.Helper()
	var buf bytes.Buffer
	w, err := modules_blockio.NewWriterWithConfig(modules_blockio.WriterConfig{
		OutputStream:    &buf,
	...
	for i, d := range defs {
		...
		if d.vec != nil {
			attrs = append(attrs, makeEmbedAttr(d.vec))
		}
		sp := &tracev1.Span{
			TraceId:           traceID[:],
			SpanId:            []byte{byte(i + 1), 0, 0, 0, 0, 0, 0, 0}, //nolint:gosec
...
// l2NormalizeF32 normalizes a float32 slice for use in plan_blocks tests.
func l2NormalizeF32(v []float32) []float32 {
	var sum float64
	for _, x := range v {
...
func TestFileLevelVectorPrune_FileLevelRejection(t *testing.T) {
	// Build a file where all spans have embedding [1,0,0].
	// File centroid will be [1,0,0]. A query vector [0,1,0] (orthogonal) has
	// cosine similarity ~0 < threshold (0.3) → file-level rejection → empty result.
	const dim = 3
	v := l2NormalizeF32([]float32{1, 0, 0})
	// Need ≥4 vectors so PQ training produces effectiveK≥2.
	defs := []vecSpanDef{
		{name: "s1", vec: v},
		{name: "s2", vec: v},
		{name: "s3", vec: v},
	...
	r := buildVectorFile(t, dim, 0, defs)

	vi, err := r.VectorIndex()
	require.NoError(t, err)
	if vi == nil {
		t.Skip("no vector index built (too few training vectors) — skip pruning test")
	}

	queryVec := l2NormalizeF32([]float32{0, 1, 0}) // orthogonal to file centroid
	program := &vm.Program{
		HasVector:   true,
		...
		Predicates: &vm.QueryPredicates{
			Nodes: []vm.RangeNode{
				{Column: "__embedding__", QueryVector: queryVec, VectorThreshold: 0.3},
			},
		},
	}
	selected := make([]int, r.BlockCount())
	for i := range selected {
		selected[i] = i
	}

	result := fileLevelVectorPrune(r, program, selected)
	assert.Empty(t, result, "orthogonal query vector should prune all blocks at file level")
}

func TestFileLevelVectorPrune_BlockLevelPruning(t *testing.T) {
	// Build a file with 2 blocks:
	// - Block 0 spans: embedding [1,0,0] (similar to query [1,0,0])
	// - Block 1 spans: embedding [0,1,0] (orthogonal to query [1,0,0])
	// Block 0 should survive; block 1 should be pruned.
	// Use MaxBlockSpans=4 to create separate blocks.
	const dim = 3
	vSimilar := l2NormalizeF32([]float32{1, 0, 0})
	vDissimilar := l2NormalizeF32([]float32{0, 1, 0})

	defs := []vecSpanDef{
		{name: "s1", vec: vSimilar},
		{name: "s2", vec: vSimilar},
		{name: "s3", vec: vSimilar},
		{name: "s4", vec: vSimilar},
		{name: "s5", vec: vDissimilar},
		{name: "s6", vec: vDissimilar},
		{name: "s7", vec: vDissimilar},
		{name: "s8", vec: vDissimilar},
	}
	r := buildVectorFile(t, dim, 4, defs) // 4 spans per block → 2 blocks
	...
	if r.BlockCount() < 2 {
		t.Skipf("expected ≥2 blocks, got %d — skip block-level pruning test", r.BlockCount())
	}

	queryVec := l2NormalizeF32([]float32{1, 0, 0})
	program := &vm.Program{
		HasVector:   true,
		QueryVector: queryVec,
		VectorLimit: 10,
	...
	result := fileLevelVectorPrune(r, program, selected)
	// File centroid is average of block centroids: roughly [0.5, 0.5, 0] → similarity ~0.7 > 0.3 → file kept.
	// Block with [1,0,0] centroid: similarity ~1.0 → kept.
	// Block with [0,1,0] centroid: similarity ~0.0 < 0.3 → pruned.
	assert.Less(t, len(result), r.BlockCount(), "at least one block should be pruned")
	assert.NotEmpty(t, result, "at least one block should survive")
}
