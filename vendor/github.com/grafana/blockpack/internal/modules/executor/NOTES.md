...
The `Stream` method has been removed — see NOTE-035. No inline block-scan loops remain in api.go.

*Note: LogQL support (StreamLogs, CollectLogs, ExecuteLogMetrics) was removed in NOTE-460.*
...
1. **Pre-compiled regex** — the `*regexp.Regexp` is compiled once at query-compile time
   (in `traceql_compiler.compileColumnPredicateComparison`) and
...
**Decision:** The second-pass `r.ParseBlockFromBytes(bwb.RawBytes, nil, meta)` has been
replaced with `r.AddColumnsToBlock(bwb, nil)`.
...
decompresses ALL columns — including those already decoded in the first pass.
This was re-doing decompression work for predicate columns that were already
present in `bwb.Block.columns`, measured at ~1.1-1.2s out of ~1.6-2.1s total
for second-pass block decodes.
...
## NOTE-025: Lazy Column Decode Handles All Columns

*Added: 2026-03-05*

**Decision:** `ParseBlockFromBytes` performs lazy registration for all columns not in
`wantColumns`. Columns not needed for predicate evaluation are registered with
presence-only decode (no zstd decompression). Their full decode is deferred to the
first value access (`StringValue`, `Uint64Value`, etc.), which calls
`Column.decodeNow()` internally.

**Rationale:** This eliminates the 90+ eager zstd decompressions per block for
non-predicate columns, reducing per-block decode cost from O(all columns) to
O(predicate columns + accessed columns).

**Safety:** The `rawEncoding` slice points into `bwb.RawBytes`, which is valid for
the lifetime of `bwb`. All lazy decodes complete within the block's row loop
iteration before `bwb` goes out of scope. Single-goroutine scan path — no locking.

Back-ref: `internal/modules/blockio/reader/block.go:Column.decodeNow`,
...
**Scope of impact:** This bug affected any TraceQL predicate using
...
and the traceql compiler was updated in the same commit. No old field names remain
in production code.
...
## NOTE-031: *(Removed — log top-K scan no longer exists after NOTE-460)*

---

## NOTE-032: aggBucketState for Trace Metrics

*Added: 2026-03-08; updated: 2026-06-30 (LogQL removal)*

**Decision:** `aggBucketState` (defined in `aggbucketstate.go`) is used by
`metrics_trace.go` for per-bucket state (`sum`, `count`, `min`, `max`, `values`).

**Back-ref:** `internal/modules/executor/aggbucketstate.go:aggBucketState`,
...
**QUANTILE — values slice + computeQuantile reuse:**
`traceUpdateBucket` appends each span's field value to `bucket.values`.
`traceRowValue` calls `computeQuantile(bucket.values, quantile)`. The `quantile float64`
parameter is threaded through `traceRowValue`'s signature.

**STDDEV — Welford online algorithm with `aggBucketState.mean` and `.m2`:**
Two new fields (`mean`, `m2`) are added to `aggBucketState` in `aggbucketstate.go`.
...
`internal/modules/executor/metrics_trace.go:traceHistogramBucket`,
`internal/modules/executor/aggbucketstate.go:aggBucketState`
...
contained spans. `Collect` returns an error instead.
...
**Decision:** All query paths (Collect, ExecuteTraceMetrics) now use a shared
`planBlocks` helper that runs:
...
Other paths called `planner.Plan` or `planner.PlanWithOptions` directly without the
`BlocksFromIntrinsicTOC` intersection step.

**Why safe:** `BlocksFromIntrinsicTOC` returns nil when no pruning is possible.
...
> **SUPERSEDED 2026-06-29 (NOTE-477):** in-file block pruning was removed entirely; the
> value index is now the sole authoritative source for pruning + block status. The file-level
> bloom reject described below (`fileLevelBloomReject`, `FileBloom`, `MayContainTraceID`) no
> longer participates in planning. `fileLevelBloomReject` was deleted from `plan_blocks.go`.
> The historical description is retained for context only.
...
from `Collect`. Public API type aliases `QueryStats` and `StepStats`
...
**Consequence:** All callers of `Collect` must update to the 3-value
return. Callers that do not need stats use `_` for the second return.
...
`ExecuteTraceMetrics`) and passed as a scratch parameter to `traceAccumulateRow`.
It clears the slice with `clear(attrVals)`
...
the pattern already used by `Stream`.
...
**Decision:** Replace the divergent block-read loops (`scanBlocks`, `forEachBlockInGroups`,
`ExecuteTraceMetrics` sequential loop, `topKScanBlocks`) with a single
`blockGroupPipeline(ctx, r, groups, workerCount, processGroup)` function.
(`topKScanBlocks` was migrated in a follow-up change; see SPEC-STREAM-11.)

**Rationale:**

1. `scanBlocks` was lazy-sequential; `forEachBlockInGroups` was eager-parallel.
   Two incompatible patterns for the same underlying operation.
2. The bounded channel (capacity=W) makes peak memory O(W × group_size ≈ W × 8MB)
   regardless of total selected blocks.
...
W=1 or W=2). All production block-scan paths (`scanBlocks`, `forEachBlockInGroups`,
`ExecuteTraceMetrics`, `topKScanBlocks`) pass `defaultPipelineWorkers = 8`
to `blockGroupPipeline` (SPEC-STREAM-11).
...
`ExecuteStructural` is exempt from SPEC-STREAM-11.
...
## NOTE-101: rowIndexScratchPool — Per-Block Scratch Slice Pool

*Added: 2026-05-15*
**Decision:** Add `rowIndexScratchPool sync.Pool` of `*[]int` in stream_topk.go.
`collectMixedTopK` (stream.go) uses `acquireRowIndexScratch` / `releaseRowIndexScratch`
to avoid per-block `make([]int, 0, N)` allocations.
**Rationale:** pprof showed `collectMixedTopK`'s
`qualifying := make([]int, 0, len(candidateRows))` are per-block allocations in every
query with a time range. The pool eliminates them.
**Cap guard:** Pool items with `cap > 65536` are replaced on return with
`make([]int, 0, 256)` to bound pool memory. Matches the pattern established by
compositeKeyScratchPool (NOTE-071).
...
Back-ref: `internal/modules/executor/stream.go:collectMixedTopK`
...
*Added: 2026-05-23*
**Decision:** Add `WantColumns map[string]struct{}` field to `vm.Program`. Populated once at compile time by `program.ComputeWantColumns()` (called at the end of `CompileTraceQLFilter`, `CompileTraceQLFilterWithOptions`, and the metrics compiler). `ProgramWantColumns` returns this cached map directly when `len(extra) == 0`. When `len(extra) > 0`, `ProgramWantColumns` copies the cached set and merges extra — O(cached-set-size + extra) — rather than re-walking the predicate tree. `ProgramWantColumns` is called from several locations across the executor package (computeColumnFilters, metrics_trace.go, predicates.go:ComputeSecondPassCols).
...
Back-ref: `internal/modules/executor/stream.go:scanBlocks`,
`internal/modules/executor/stream_topk.go:topKScanBlocks`
...
`collectStreamInto` reuses `p.scratch` for the first Scan*call within a `ColumnPredicate`
evaluation. `releaseBlockColumnProvider` resets both `scratch` and `scratchInUse`.
**Rationale:** pprof showed `newRowSetWithCap` contributing allocs in every Scan* path.
For single-predicate queries (the common case), the scratch eliminates one `[]int` alloc
per block.
**Lifetime invariant:** The rowSet returned by the first `collectStreamInto` call borrows
`p.scratch`. `releaseBlockColumnProvider(p)` must be called AFTER all reads of the rowSet
are complete. All call sites follow: acquire → ColumnPredicate → consume rowSet → release.
**Call-site audit (all callers confirmed safe):**

+ `stream.go:scanBlocks` — release after `rowSet.ToSlice()`. SAFE.
+ `stream.go:collectMixedPlain` — provider released at end of fn or in preFn. SAFE.
+ `stream.go:collectMixedTopK` — same preFn/fn pattern as collectMixedPlain. SAFE.
+ `stream_topk.go:topKScanBlocks` — release after `rowSet.ToSlice()`. SAFE.
+ `metrics_trace.go` — release after `rowSet.ToSlice()` loop completes. SAFE.
+ `stream_structural.go:processBlock` — release after the row loop. SAFE.
...
`memo := make([]uint8, len(spans))` — a fresh tri-state
(memoUnknown/memoYes/memoNo) ancestor
...
the last unpooled structural memo. The pool stores `*[]uint8` (pointer-wrapped)
so `Put` is
...
pool. Output is byte-identical (same memo states, same emit order);
the only change is the backing
...
released via `defer` before return. The defers fire after the named return values are
evaluated, and
...
Dead-code cleanup (rules: no dead code): `storeTypedField` (unexported)
had no remaining caller and
...
## NOTE-423: dict intrinsic columns scatter directly from DictEntries

— skip the sorted refIndex build

*Added: 2026-06-16*

`populateTypedColumnForBlock` (NOTE-100) fills one intrinsic column's values into the
per-block
...
indexed by the ref's low-16 `RowIdx`; it is fully order-INDEPENDENT and
never does a reverse
...
emits one write `result[ref.RowIdx] = value`.
That is O(totalRefs) **per block call** because
...
Reader holds the whole object file — many internal blocks — and
structural `{a} >> {b}` queries
...
+ **Multi-block** dict column → build the sorted refIndex **once** (cached via `EnsureRefIndex` /
  ...
  O(log totalRefs + blockRefs) per block.
New `*DictRefIndex` scatter helpers consume that window:
  `entry.Pos` is the dict-entry index, `entry.Packed&0xFFFF` is the row index, so
  `result[rowIdx] = DictEntries[Pos].value`. Same (rowIdx, value) pairs as the
  direct scatter for
  this block, sourced from the cached sorted index instead of an O(totalRefs) re-walk.

The one-time `DictMultiBlock` scan costs the same O(totalRefs) the first block's
direct scatter
...
(`[][]byte`) into the `[8]byte`/`[16]byte` field on `intrinsicRowFields`.
The previous code used
`copy8(dst, b)` (and `copy(dst[:], b)` for trace IDs), whose `copy` builtin computes
`min(len(dst), len(b))` and lowers to a runtime memmove-style sequence
even though the preceding
`len(b) == N` guard fixes the length. This per-row copy was the #1 blockpack self-time frame on
the structural Q9 profile (`copy8`, ~0.72s) because it runs O(SpanCount)
per block over the union
of selected blocks.
...
the guard already proves), eliminating the builtin's min-length computation
and memmove dispatch.
...
`len(b) == N` guard, because the guard's proven length is not propagated
into the builtin.
...
that drives the Q9 9x-vs-parquet gap), the old `computeNodeMatchForRow`
probed EACH program's
`vm.RowSet` with `rowSet.Contains` — a `slices.BinarySearch` — once per row.
Cost was
O(spanCount × programs × log(matchedRows)) across the block, the dominant per-block CPU on the
structural path.

**Change:** Each `RowSet` is already a sorted ascending slice of its matched rows.
Scatter each
set's matched rows directly into a per-row `[]uint8` bitmask in a single
linear pass per program
(`computeStructuralPredBits`, O(sum of set sizes) — visits only matched rows,
typically far fewer
than spanCount). The row loop then reads bit `i` with one array index instead of a binary search.
`computeNodeMatchForRow` now takes the precomputed `predBits uint8` for that row
and only folds in
...
+ `Collect` gains `ctx context.Context` first param (NOTE-058 resolved).
...
## NOTE-477: In-file block pruning removed — value index owns pruning + block status

*Added: 2026-06-29*

All in-file block-level pruning structures were removed from the blockpack format. The value
index (V4 entries, per-column, page-addressed `BlockRef`) is now the single authoritative source
for both pruning and block status. The "v2 lean format" toggle (`EnableV2Format`/`FooterV9`) was
collapsed to unconditional: every file is FooterV9 with 4 KB page-aligned blocks; FooterV8 read
support was dropped (no legacy files to read).

**Removed from `plan_blocks.go`:**

+ `pruneByColStats` + `colStatsRejects*` helpers (NOTE-446 ColStats pruning).
+ `fileLevelBloomReject` + bloom-reject helpers (NOTE-045).
+ `PrunedByColStats` telemetry.

**Removed from the writer:** ColStats section (`writeV8FileSections`), per-file TS index, file
bloom, KLL sketch (already #435), IntrinsicTOC (already #433). **Removed from the reader:**
`ColStats`/`HasColStats`, `IsV2Format`/`FooterVersion`, FooterV8 acceptance; `BlocksInTimeRange`
returns nil (no TS index) so the planner falls back to `BlockMeta` MinStart/MaxStart for
time-range pruning. **Shared:** `colstats.go` deleted; `ToCSubTypeColStats`/`FooterV8Version`
retired.

**Correctness:** the value index is policy-free and indexes every column (NOTE-VI-018/#414,
nil denylist), so value-index-only pruning + full block scan is correct for all query shapes.
Accepted regression: `!=` string queries have no fast value-index negation path and fall back to
scan.

Supersedes NOTE-045 (this file), NOTE-446 (writer + reader), NOTE-448, and the `PrunedByColStats`
path in queryplanner NOTE-449.

Back-ref: `internal/modules/executor/plan_blocks.go:planBlocks`,
`internal/modules/blockio/writer/v8_sections.go:writeV8Sections`,
`internal/modules/blockio/reader/parser.go:tryReadFooterMagic18`

## NOTE-VI-033: Zero-block-read metrics path — count_over_time/rate from value index

*Added: 2026-06-30*

`ExecuteTraceMetricsFromVI` (issue #460) answers `count_over_time()` and `rate()` **without
group-by** entirely from value-index data — no blockpack file reads. The value index carries
`(TraceID, SpanID, TimeSec)` per indexed span per column, which is everything needed to count
distinct TraceIDs per time bucket.

**Gate (returns `ok=false` → caller falls back to `ExecuteTraceMetrics` block scan):**

+ Function is not COUNT/RATE (e.g. `max_over_time`, `histogram_over_time`).
+ Any group-by clause (`by (...)`) — VI entries carry no attribute values, only identity+time.
+ Any leaf column with **no index coverage** (the source's `LookupResults` returns `ok=false`).
+ Match-all (`{}`) over an empty source (`AllResults` returns `ok=false`).
+ Any matched span with `TimeSec == 0` (file predates per-span timestamps → unknown bucket).

**Predicate tree walk** (`viMatchSpans`/`viEvalNodes`): mirrors the block-scan boolean logic.
...
**empty slice** for every column it queried-but-found-no-files-for; `LookupResults` then returns
`(nil, true)` (coverage, empty) rather than `(nil, false)` (no coverage → fall back). A column the
caller never queried is absent from the map → `(nil, false)` → fallback. `SliceValueIndexSource`
encodes this by map-key presence.

**Counting semantics:** distinct TraceIDs per bucket (a trace with N spans counts once per
bucket), matching the block-scan accumulator's no-group-by dense series. `rate` divides the bucket
count by step-seconds. Window is right-closed `(StartTime, EndTime]`.

**Wiring:** `ExecuteMetricsTraceQL` (api.go) tries the VI path first when
`TraceMetricOptions.ValueIndex != nil`, falling back on `ok=false`. The querier supplies the
source in issue #461; until then the field is nil and the path is a structural no-op.

Back-ref: `internal/modules/executor/metrics_trace.go:ExecuteTraceMetricsFromVI`,
`api.go:ExecuteMetricsTraceQL`, `tracemetricoptions.go:TraceMetricOptions.ValueIndex`.
Tests: `metrics_trace_vi_test.go`.

## NOTE-VI-035: Index-driven search path — QueryTraceQLFromIndex (issue #459)

*Added: 2026-06-30*

`QueryTraceQLFromIndex` (executor + `api.go` wrapper) answers a TraceQL **filter**
query using value-index data for block pruning: it resolves the matching
`(SourceRef, BlockID, RowIdx)` spans from a pre-populated `ValueIndexSource`, then
fetches **only the blocks that contain matches** (coalesced `Reader.ReadBlocks`) and
materializes their fields with a column-restricted `WantOnly` parse — no full-file
scan, no decode of unreferenced columns.

**Mirrors the metrics path (NOTE-VI-033) exactly** for discovery/coverage: the
querier (issue #461) discovers + downloads + applies the per-leaf predicate to the
VI files for the Reader's `SourceRef` and populates the source; this function just
consumes it. The predicate tree walk is the shared `viMatchSpans`/`viEvalNodes`
(AND intersect, OR union, match-all = `AllResults`), so search and metrics agree on
boolean semantics.

**`VILookupResult` gained `BlockID` (uint32) + `RowIdx` (uint16)** so the search path
has O(1) direct row access. The metrics path ignores both.

**Fallback (`ok=false` ⇒ caller falls back to `QueryTraceQL` full scan):**
...
definitive empty result for this file, NOT a fallback. Same map-key-presence coverage
encoding as `SliceValueIndexSource` (NOTE-VI-033).

**Wiring:** until issue #461 the querier passes no source, so `QueryTraceQLFromIndex`
is never invoked — `QueryTraceQL` (full scan) remains the only search path. The change
is purely additive (new exported func + two new `VILookupResult` fields).
...
## NOTE-VI-087 — compileStructuralPair: 2-node-only v1 compile bridge for index-driven structural queries (issue #489, plan-d.md §D2)

**What this is.** `compileStructuralPair(q *traceqlparser.StructuralQuery) (leftProg, rightProg *vm.Program, op traceqlparser.StructuralOp, ok bool, err error)` is the first compile-path step of #489's index-driven structural query engine: it takes a parsed `StructuralQuery` and produces two independently-compiled `*vm.Program` filter legs plus the operator between them, reusing `traceqlparser.FlattenChain` (already exported) and `vm.CompileTraceQLFilter` per leg — the same per-leg compile step `stream_structural.go`'s existing unexported `compileStructuralPrograms` already performs for the scan path, just returning a typed 2-tuple instead of a slice.

**2-node-only v1 scope (deliberate carve-out, not a silent gap).** A query that flattens (via `FlattenChain`) to anything other than exactly 2 filter nodes and 1 operator is a routine DECLINE: `ok=false, err=nil`. The caller falls back to the existing scan path (`ExecuteStructural`), which already supports N-ary chains up to 8 nodes (SPEC-STRUCT-8). This mirrors the project's established routine-decline-vs-error distinction (see `queryplan/NOTES.md` NOTE-QP-006 and `valueindex/NOTES.md` NOTE-VI-072 for the same shape of distinction elsewhere in the codebase: a caller-visible "this path doesn't apply, fall back" signal must never be conflated with a genuine error). Lifting to N-ary chains is an intentional, clean follow-up once the 2-node path is proven end-to-end — not deferred-and-forgotten scope.

**Compilation does not gate on positive-vs-negated operator.** `{A} !>> {B}` compiles successfully through `compileStructuralPair` — the function only cares about chain SHAPE (2 nodes, 1 op), never the op's polarity. Declining negated operators is an execution-layer decision made downstream (plan-d.md §D4, `ExecuteStructuralFromIndex`), not here — this keeps `compileStructuralPair` reusable unchanged by §D6's negated-structural engine, which needs the exact same two compiled legs and operator, just with different execution-time handling.

**Nil filter leg (match-all) compiles to a nil `*vm.Program`.** `compileStructuralLeg` mirrors `compileStructuralPrograms`' existing convention exactly (nil filter -> nil program) rather than introducing a new sentinel value — `{} >> {span.x = "y"}`'s left leg is `nil`, not a distinguished "match-all program."


**Informational aside (no ID assigned — not spec-worthy, no invariant violated, no public contract changed).** During D2's test-writing, a parser oddity surfaced: `traceqlparser.ParseTraceQL("{ span.a =~ \"(\" } >> { span.b = \"y\" }")` (malformed regex `"("` as the LHS filter's pattern) does not error out as an unparseable structural query and does NOT return a `*traceqlparser.StructuralQuery` — it falls back to returning a plain `*traceqlparser.FilterExpression` instead, silently, before structural-vs-filter classification is ever attempted. Suspected root cause (uninvestigated): quoting/bracket-tokenizer interaction between the structural `{ } >> { }` delimiter scanning and the malformed regex's unbalanced `(` confusing the parser's brace-matching. `compileStructuralPair`'s own `TestCompileStructuralPair_MalformedFilterPropagatesCompileError` test uses a different malformed pattern (`"["`) inside a STILL-recognized 2-node structural query and passes fine — this is a distinct, parser-level (not compile-level) issue that never reaches `compileStructuralPair` at all. Out of scope for Phase D (`traceqlparser`'s tokenizer/parser is untouched by this phase); flagged here for whoever next works on `traceqlparser` (`#481`'s scan-removal work touches adjacent parser-consumption code, or a future dedicated parser fix) to investigate and decide whether it is a bug (silently wrong classification) or an acceptable fallback (a malformed-enough LHS makes "is this even a structural query" ambiguous, and falling back to a filter expression that will itself fail to compile for the same reason is arguably a soft landing). No test currently locks in either behavior as intended.

Back-refs: `internal/modules/executor/structural_compile.go:compileStructuralPair, compileStructuralLeg, CompileStructuralLegs`, `internal/modules/executor/stream_structural.go:compileStructuralPrograms` (the existing scan-path convention this mirrors), `internal/traceqlparser/parser.go:FlattenChain`. Spec invariants referenced (not modified): SPEC-STRUCT-8 (N-ary chain support on the scan path, the fallback target for the decline case), SPEC-ROOT-001 (no panic — decline, not error, on out-of-scope shape). Tests: `structural_compile_test.go` (`TestCompileStructuralPair_TwoNodeDescendant_CompilesBothSides`, `_NilFilterCompilesToNilProgram`, `_ThreeNodeChainDeclines`, `_MalformedFilterPropagatesCompileError`, `_NegatedOpStillCompiles`). Issue #489.

---

## NOTE-VI-090 — verifyCandidateSpans: candidate verification via targeted row reads is NEW execution-strategy work, not a reuse of any #484 Phase 3 mechanism (issue #489, plan-d.md D3B)

**What this is.** `verifyCandidateSpans(ctx, prog *vm.Program, spans []ResolvedSpan) ([]ResolvedSpan, error)` (`internal/modules/executor/structural_verify.go`) evaluates `prog` against every span in a candidate set and returns only the surviving ones, using EACH span's own resolved reader (`ResolvedSpan.Reader`, NOTE-VI-088) rather than a single shared reader — because a multi-file candidate trace (D3, Option A) may need confirming reads across several files. Candidate spans sharing a `(Reader, BlockIdx)` are grouped (`groupResolvedSpansByBlock`) so a block referenced by several candidates is fetched and parsed exactly once, then evaluated via `evaluateProgramAgainstBlock` — the same `vm.Program.ColumnPredicate` + `acquireBlockColumnProvider` pairing `stream_structural.go`'s own `collectBlockStructuralSpanRecs` already uses for per-block predicate evaluation on the scan path, applied here to one targeted block instead of a whole query's block set. This is a new CALLER of an existing evaluation primitive, not a new VM evaluation mechanism.

**This is CONFIRMED NEW WORK — issue #489's own text is INACCURATE on this point (binding historical-record correction).** Issue #489's step 4 describes this as "verify remaining leaves against candidates, reusing #484 Phase 3's cost-based threshold." Direct code read of `vibuilder.BuildSource` (brainstormer-d, confirmed by the team lead) found no such "verify-against-candidates, cost-threshold-gated" mechanism anywhere in `vibuilder.BuildSource`'s AND-group leaf resolution, or anywhere else in the codebase — `queryplan.ClassifyWithThreshold`/`ClassifyProgramVCNT` (NOTE-QP-002/003) remain reusable for a DIFFERENT, coarser decision ("is this side worth VI-resolving at all") but say nothing about how a narrowed candidate set's remaining, NOT-VI-resolved side gets confirmed. `verifyCandidateSpans` is designed and implemented here as its own first-class deliverable. A future reader of issue #489 alone, without this note, could wrongly conclude an existing #484 Phase-3 mechanism was reused — it was not.

**A second, independent inaccuracy in plan-d.md itself (also worth recording).** Plan-d.md's own D3B.2 implementation hint claimed `verifyCandidateSpans` should reuse `modules_blockio.NewSpanFieldsAdapterWithReader`, describing it as "the same primitive `QueryTraceQLFromIndex`'s materialization step already wires together" with a per-row VM predicate evaluation. This combination does not exist anywhere in the codebase — `QueryTraceQLFromIndex` relies entirely on the value index having already applied predicates and never runs a VM program per-row at all. The actual reused mechanism (`Program.ColumnPredicate` + `acquireBlockColumnProvider`, described above) was substituted after this was discovered to be inaccurate.

**Consumers.** D4 (positive structural operators) needs `verifyCandidateSpans` whenever a structural query's non-lead filter leg is too expensive to VI-resolve outright; D6 (negated structural operators, `!>>`/`!>`/`!~`) needs it UNCONDITIONALLY — a negated operator's left side can never be VI-led at all, so confirmation via targeted row reads is the ONLY resolution path available for that side, not an optimization over an alternative.

**Design details:**
- A `nil prog` matches every span (mirrors `compileStructuralPair`'s NOTE-VI-087 match-all convention).
- An empty `spans` list returns `(nil, nil)` immediately, no I/O.
- Any resolve failure (a candidate naming a block absent from its own reader, or a parse/evaluate failure) is a hard error, never a silent drop — the same discipline `MaterializeTraceGroupMultiFile` (NOTE-VI-088) already applies to materialization itself.
- The returned slice preserves the input spans' original relative order.
- Unexported (`verifyCandidateSpans`, lowercase) — only D4 and D6, both in `internal/modules/executor`, call it; no public contract beyond this NOTES.md entry.

Back-refs: `internal/modules/executor/structural_verify.go:verifyCandidateSpans, groupResolvedSpansByBlock, evaluateProgramAgainstBlock, verifyBlockKey`, `internal/modules/executor/stream_structural.go:collectBlockStructuralSpanRecs` (the existing fetch/parse/`ColumnPredicate` sequence this reuses per targeted block), `internal/modules/executor/structural_multifile.go:ResolvedSpan` (NOTE-VI-088, the input shape), `internal/modules/queryplan/{selectivity,vcnt_cost}.go:ClassifyWithThreshold, ClassifyProgramVCNT` (NOTE-QP-002/003, the DIFFERENT coarser decision these remain reusable for — not this confirmation mechanism). Tests: `internal/modules/executor/structural_verify_internal_test.go` (`TestVerifyCandidateSpans_NilProgramMatchesAll`, `_FiltersToMatchingRowsOnly`, `_MultiFileTrace_UsesEachSpansOwnReader`, `_EmptySpanListReturnsEmpty`, `_ReaderReadErrorSurfacesAsError`). Issue #489.

---

## NOTE-VI-091 — FindTraceGroupInCandidates: relocated candidate-TraceGroup merge/lookup algorithm, shared by GetTraceByID and ExecuteStructuralFromIndex (issue #489, plan-d.md D4, team-lead ruling 2026-07-07)

**What this is.** `FindTraceGroupInCandidates(ctx, lister valueindex.LookupStore, keys []string, traceID [16]byte, queryMinSec, queryMaxSec uint64) (valueindex.TraceGroup, bool, error)` (`internal/modules/executor/structural_tracegroup.go`) fetches and decodes every candidate trace-by-id index file in `keys`, merging every matching group found for `traceID` across ALL of them (spans deduplicated by SpanID, first occurrence wins; `TimeSec` is the minimum across matches — the same live-merge semantics as `valueindex.MergeTraceGroups`). Candidates are NOT short-circuited on the first match: `DiscoverIndexFiles` can legitimately return multiple valid, not-yet-compacted L0 files for the same TraceID with DISJOINT `Spans` (e.g. a trace's root span flushed in one consumer window, its child span in another, before compaction) — stopping early would silently return a partial trace.

**Relocated from root under the same rule as D3's ResolveTraceGroupSourceRef extraction (NOTE-VI-089).** This has zero root-package dependency (`valueindex` only), and both `GetTraceByID`'s single-trace lookup and `ExecuteStructuralFromIndex`'s (NOTE-VI-092) many-candidate-trace discovery need it — so it is the one canonical implementation here; root's `findTraceGroupInCandidates` (`reader.go`) is now a thin delegate. This is the THIRD primitive extracted from root's `GetTraceByID` machinery for #489 (alongside `ResolveTraceGroupSourceRef`, NOTE-VI-089, and `MaterializeTraceGroupMultiFile`, NOTE-VI-088) — all three exist so `GetTraceByID` and the structural index-driven engines can never maintain independently-drifting copies of the same authoritative-index semantics.
...
## NOTE-VI-107 — SliceValueIndexSource.LookupLeaf: leafAware gate stops a declined leaf from borrowing a covered sibling's results (task #207, 2026-07-12)

**The bug.** Issue #206 introduced `AddLeaf`/`LookupLeaf` to disambiguate two leaf `RangeNode`s on the SAME column (e.g. `score >= 100 && score <= 150`) by keying a per-leaf bucket on the leaf's own structural DFS position, instead of the shared column-name aggregate `Add`/`LookupResults` use. `LookupLeaf`'s original fallback — "no per-leaf entry for `leafIdx`? fall back to `LookupResults(colName)`" — was written to keep every LEGACY (never leaf-keyed) caller byte-identical, but it could not distinguish two very different reasons for a missing entry: (a) this source never used leaf-aware keying at all, so the aggregate IS the only data (correct fallback), vs. (b) THIS SPECIFIC leaf was declined at build time (task #204's millisecond-decidability gate on `span:duration`, a `RequirePresent` leaf from a scoped `!=`, or any other `vibuilder.buildPredicate` decline) while a SIBLING leaf on the same column WAS covered and wrote into that same aggregate bucket. Case (b) silently substituted the sibling's own match set for the declined leaf — `viEvalAND` then intersected a leaf with a copy of itself (dropping the declined leaf's constraint, false positives), and `viEvalOR` unioned a leaf with a copy of itself (also dropping the constraint, but false negatives instead). This is issue #206's own union-instead-of-intersection failure mode reintroduced through a different path than #206 itself fixed.

**Independent discovery.** Confirmed fresh by `value_index_oracle_comparison_test.go`'s Shape 9a (AND, false-positive direction) and Shape 9b (OR, false-negative direction — deliberately built with a non-superset condition pair so a wrong union/intersection substitution is independently observable in both directions, not masked by monotonic-superset coincidence). Shape 9c (different-column decline) was already a genuine pass and stayed that way: a declined leaf whose column has no other leaf anywhere in the query has nothing to borrow from — `LookupResults` legitimately returns `(nil, false)` on its own, no gate needed.

**The fix.** Added `SliceValueIndexSource.leafAware bool`, set to `true` inside `AddLeaf` the moment it is called with a valid (`>= 0`) `leafIdx`, for ANY leaf. `LookupLeaf` now only falls back to `LookupResults` when `leafAware == false` (case (a)) — once ANY leaf of the query has been keyed positionally, EVERY leaf sharing that source is resolved leaf-specifically, and a missing entry unconditionally means "not covered" (`nil, false`), never a fallback (case (b), the fix). This is exactly the gate the task's root-cause direction proposed: "track whether AddLeaf was EVER called for this instance; if leaf-aware keying was used for ANY leaf, every leaf lookup must go through leaf-specific resolution."

**Side effect on Shape 11(a)/(c) (worth recording, not a regression) — SUPERSEDED, see the task #212 addendum below.** Both shapes contain a `RequirePresent` leaf (from a scoped `!=`'s `[RequirePresent(col), OR{col>x, col<x}]` compilation) sharing a column with a covered sibling. Before this fix they passed by a fragile, shape-specific COINCIDENCE: the borrowed aggregate happened to equal the sibling's own value for exactly these tree shapes (no other same-column leaf existed elsewhere in the query), so intersecting/unioning with a duplicate of yourself was a harmless no-op. After this fix (but BEFORE task #212's own addendum below), `RequirePresent` leaves in a leaf-aware source correctly declined instead of borrowing — both shapes passed for a GENUINE reason (the whole query declined to the block-scan fallback) rather than a coincidental one, at the cost of declining EVERY `attr != V` metrics/search query on a non-built-in column, a real regression task #212 fixes. `value_index_oracle_comparison_test.go`'s Shape 11a/11c were renamed `...ResolvesCorrectly` (previously `...DeclinesCorrectly`) and now assert the correct VI-path answer again, not a decline — see the addendum immediately below for why this is no longer a coincidence either.

**No new public API.** `AddLeaf`/`LookupLeaf` themselves were already signed off for issue #206; this fix adds only a private field (`leafAware`) and changes `LookupLeaf`'s internal decision logic — the exported `ValueIndexSource` interface, `SliceValueIndexSource`'s exported method set, and every existing caller's call site are unchanged.

Back-refs: `internal/modules/executor/metrics_trace.go:SliceValueIndexSource.AddLeaf, SliceValueIndexSource.LookupLeaf, SliceValueIndexSource.leafAware`. See SPEC-VIS-6 (`SPECS.md`) for the "what" contract. Tests: `value_index_oracle_comparison_test.go` Shapes 9a/9b/9c/11a/11c. Issues #206, #207.

### Addendum (task #212, 2026-07-12) — RequirePresent carve-out: a declined leaf CAN safely borrow a covered sibling's results, when the sibling proves the exact same fact

**The regression this note's own fix caused.** NOTE-VI-107's fix above is *too* conservative for exactly one shape: `attr != V` on a non-built-in (dedicated/custom) column always compiles to `[RequirePresent(attr), OR{attr>V, attr<V}]` (`extractNeqNode`/`extractNeqNumericNode`, NOTE-453/454) — a `RequirePresent` leaf that `vibuilder.buildPredicate` NEVER builds a value predicate for (its own `default` branch: "RequirePresent or empty leaf: existence-only, not a value predicate"), paired with a REAL range-OR sibling on the SAME column that DOES get indexed. Once the range-OR sibling's own leaves `AddLeaf` and make the source leaf-aware, `LookupLeaf`'s blanket decline (this note's own fix) fires the moment it reaches the `RequirePresent` leaf's own (deliberately never populated) `leafData` entry — declining the WHOLE query, even though the sibling's own data already proves everything the `RequirePresent` leaf needs: presence-on-a-column is implied by ANY successful indexed lookup on that same column (a span cannot appear in the value index for column X without X being present). This broke every `count_over_time()`/`rate()` and search `!=` query on a non-built-in column (`ErrMetricsNoCoverage` for metrics), confirmed via a real end-to-end write-path test (`neq_metrics_realvi_test.go`) and via `value_index_oracle_comparison_test.go`'s own Shape 11a/11c (which, per the paragraph above, this note's fix had turned into declines).

**Why a blanket revert is not the fix.** Reverting to the pre-#207 `LookupLeaf` fallback (fall back to `LookupResults(colName)` on ANY missing leaf entry) would silently reopen issue #206/#207's own union-instead-of-intersection bug for a genuinely different same-column decline (e.g. task #204's millisecond-decidability gate on `span:duration` — Shape 9a/9b/9c must keep declining). The fix must be narrower than either extreme: distinguish "this leaf is a `RequirePresent`-only leaf whose column already has real coverage from an indexed sibling" (safe to borrow) from "this leaf was declined for any other reason" (must still decline).

**The fix.** `SliceValueIndexSource.MarkRequirePresentLeaf(leafIdx int, colName string)` — a new exported method — records that `leafIdx` is a `RequirePresent`-only leaf. `vibuilder.BuildSource`/`BuildSourceBounded`'s leaf loop calls it (instead of a bare `continue`) when `buildPredicate` declines a leaf AND `leaves[i].node.RequirePresent` is true. `LookupLeaf` then, on a missing `leafData[leafIdx]` entry in a leaf-aware source, checks: is `leafIdx` marked `RequirePresent` AND does its column have ANY data in `s.data` (written there unconditionally by every `AddLeaf` call, including the real sibling's)? If both hold, return that column's own aggregate (`LookupResults(colName, colType)`) — safe by the same reasoning NOTE-VI-107's own bug report gives for why borrowing was ever a no-op for this shape, except now it is a DELIBERATE, narrowly-scoped mechanism instead of a coincidence. Otherwise (missing, leaf-aware, not marked, or marked but the column has no data at all — every leaf on that column was itself declined) the decline is unchanged from NOTE-VI-107's own fix.

**New public API surface, explicitly flagged.** `SliceValueIndexSource.MarkRequirePresentLeaf` is a new EXPORTED method — the third in this same leaf-identity bookkeeping family after `AddLeaf`/`LookupLeaf` (issue #206) and `MarkNewestFirst` (Phase 4). Per this repo's established pattern for this struct in this cascading investigation, this does not require a fresh permission round — flagged here, and in the task #212 completion report, for independent verification.

**Verification.** (1) A real end-to-end `count_over_time()`/`rate()` `!=` metrics test through the real write path, real compiler, and real `BuildValueIndexSourceForMetrics`/`ExecuteMetricsTraceQL` entry points (`neq_metrics_realvi_test.go`), confirmed to fail with `ErrMetricsNoCoverage` before this fix (via a temporary, reverted-after-confirming local edit) and pass with correct counts/rates after. (2) `value_index_oracle_comparison_test.go`'s Shape 9a/9b/9c (genuine same-column decidability declines) re-run explicitly and confirmed still declining — the carve-out's `s.data[colName]` existence check does not readmit issue #206's bug for those shapes (no real sibling ever writes `s.data` for the declined column in 9a/9b's case; 9c's declined column has no sibling leaf on it at all, same as before). (3) A built-in-column audit (`extractNeqNode`'s `isBuiltInField` branch) confirmed built-in `!=` never reaches this mechanism at all — a standalone built-in `!=` compiles with `Predicates == nil` entirely, and an AND'd built-in `!=` compiles to `Nodes: nil, Columns: [...]`, task #211's own "declined but column-referencing" shape, which never reaches `buildPredicate`/`MarkRequirePresentLeaf` (`TestBuildSource_BuiltInFieldNotEquals_ANDWithLeaf_DeclinesWithoutRequirePresent`, `internal/modules/vibuilder/builder_test.go`).

Back-refs: `internal/modules/executor/metrics_trace.go:SliceValueIndexSource.MarkRequirePresentLeaf, SliceValueIndexSource.LookupLeaf, SliceValueIndexSource.requirePresentLeaves`, `internal/modules/vibuilder/builder.go:BuildSource, BuildSourceBounded`. Tests: `value_index_oracle_comparison_test.go` Shapes 9a/9b/9c/11a/11c, `neq_metrics_realvi_test.go`, `internal/modules/vibuilder/builder_test.go:TestBuildSource_BuiltInFieldNotEquals_ANDWithLeaf_DeclinesWithoutRequirePresent`. Issue #212.

### Addendum (task #213, CRITICAL regression fix, 2026-07-12) — the task #212 carve-out's safety argument was true only for the SCOPED `!=` rewrite; corrected to key by paired sibling leaf INDEX, never by bare column name

**The regression the task #212 addendum above caused.** That addendum's safety argument — "the `RequirePresent` leaf's column always has a REAL sibling leaf... `AddLeaf`'d whenever the query has any coverage" — is TRUE ONLY for the SCOPED `!=` rewrite (`span.attr != V` / `resource.attr != V`, `[RequirePresent(col), OR{col>V, col<V}]`, both from the SAME rewrite call on the SAME column) and FALSE for the UNSCOPED rewrite (bare `.attr != V`, no scope prefix), which compiles to `OR{RequirePresent(resource.attr), RequirePresent(span.attr)}` — deliberately presence-only, with NO value-bearing sibling at all for either scope (composing a per-scope range-OR across two scopes was judged not worth the complexity; see `extractNeqNode`'s own unscoped-branch doc comment, unchanged by this fix). `MarkRequirePresentLeaf`'s original `colName`-keyed bookkeeping could not tell these two shapes apart: `unscopedCols` produces the EXACT same expanded column names (`resource.<name>`, `span.<name>`) that a completely unrelated, independently-authored leaf ELSEWHERE in the same query might reference — e.g. `{ .score != 100 && resource.score > 5 && span.score > 10 }`, where `resource.score > 5` and `span.score > 10` are ordinary leaves on the identical column names the unscoped `!=`'s two `RequirePresent` leaves target. `LookupLeaf`'s carve-out then substituted THAT unrelated leaf's arbitrary match set as "proof of presence" — neither a superset nor an exact match of true presence, and nothing else in the tree can correct it, because the unscoped shape's `OR{RequirePresent, RequirePresent}` IS the complete representation for that condition. This gap is exactly why the regression was not caught until adversarial review: two independent reviewers (review-consolidator, go-presubmit-reviewer), in parallel passes over the SAME diff, found the SAME bug via two different reproduction shapes — a false positive via AND (an unrelated superset-shaped collision silently satisfying the `!=` clause for a trace that should have failed it) and a false negative via OR (an unrelated collision's match set silently replacing a genuinely-matching trace's own presence proof, dropping it from the union).

**Why a blanket revert (back to task #207's unconditional decline for every `RequirePresent` leaf) is not the fix.** That would reopen exactly the regression task #212 fixed: every scoped `attr != V` metrics/search query on a non-built-in column would decline again (`ErrMetricsNoCoverage` for metrics), even though the scoped shape's carve-out IS safe — it was never the scoped case that was broken.

**The fix.** `vm.RangeNode` gained a new field, `NeqPairedRange bool` — set ONLY by `extractNeqNode`'s scoped, non-`isIntrinsicRefsColumn` branch and by `extractNeqNumericNode`'s scoped branch, on the `RequirePresent` leaf itself, marking that the very next element of the SAME returned node slice is the value-bearing range-OR composite from that exact same rewrite call. NEVER set by either function's unscoped branch (no sibling exists), nor by the scoped string branch's `isIntrinsicRefsColumn` skip (the range-OR append is skipped entirely for those columns, so there is no sibling either).

`vibuilder`'s `collectLeaves` (`builder.go`) uses this bit — and only this bit — to compute a leaf's `pairedLeafIdxs []int`: when a leaf is `RequirePresent && NeqPairedRange`, `neqRangeSiblingLeaves` validates that the immediately next sibling in the SAME node slice is EXACTLY the expected shape (an OR composite with exactly two direct leaf children, both on the same column — never assumed from bare positional adjacency alone) and predicts the DFS leaf-slot indices its two children will be assigned (a composite node never consumes a leaf slot itself, so the sibling's own two leaves are guaranteed to land at the next two slots). `BuildSource`/`BuildSourceBounded`'s leaf loop now calls `MarkRequirePresentLeaf(leafIdx, pairedLeafIdxs)` — SIGNATURE CHANGED from `MarkRequirePresentLeaf(leafIdx int, colName string)` — only when `pairedLeafIdxs` is non-empty; the unscoped rewrite's `RequirePresent` leaves never get a non-empty `pairedLeafIdxs`, so this method is never called for them at all, and they fall through to the ordinary, unconditional decline exactly like pre-#212 (#207-only) behavior.

`SliceValueIndexSource.requirePresentLeaves` is now keyed `leafIdx -> []int` (the paired sibling leaf-slot indices), not `leafIdx -> colName`. `LookupLeaf`'s carve-out now unions ONLY `s.leafData[pairedIdx]` for each `pairedIdx` in the paired set — never `s.data[colName]`, the bare column-name aggregate any unrelated leaf on that column could have populated. If any paired sibling lacks its own `leafData` entry, the carve-out does not fire and the leaf falls through to the ordinary decline.

**New/changed public API surface, explicitly flagged.** `SliceValueIndexSource.MarkRequirePresentLeaf`'s signature changed from `(leafIdx int, colName string)` to `(leafIdx int, pairedLeafIdxs []int)`. Per this repo's established pattern for this exact struct in this cascading investigation (user-approved for `AddLeaf`/`LookupLeaf`, issue #206; `MarkRequirePresentLeaf` itself, task #212), a signature change to an already-approved method for the same bug-fix lineage does not require a fresh permission round — flagged here for independent verification.

**Verification.** (1) Two real end-to-end reproduction tests through the real write path, real compiler, and real `BuildValueIndexSource`/`QueryTraceQLFromIndex` entry points (`neq_unscoped_collision_realvi_test.go`), both confirmed to fail for the predicted wrong reason before this fix (verified via a temporary, reverted-after-confirming local edit reverting `vm.RangeNode.NeqPairedRange` and this fix's other three files back to task #212's own shape) and to decline cleanly to the block-scan fallback after. (2) The full `value_index_oracle_comparison_test.go` suite (all 22 shapes, including 9a/9b/9c's declines and 11a/11c's scoped-`!=` resolves) and `neq_metrics_realvi_test.go` (task #212's own scoped-`!=` regression test) re-run and confirmed still passing — the scoped rewrite's carve-out is unaffected. (3) `structural_oracle_comparison_test.go`'s new Shape 7 confirms the identical unscoped-collision shape, reached through `ExecuteNegatedStructuralFromIndex`'s right leg (which shares `vibuilder.BuildSource` + `viMatchSpans` with the filter path), also declines cleanly rather than silently dropping a true match.

Back-refs: `internal/modules/executor/metrics_trace.go:SliceValueIndexSource.MarkRequirePresentLeaf, SliceValueIndexSource.LookupLeaf, SliceValueIndexSource.requirePresentLeaves`, `internal/modules/vibuilder/builder.go:collectLeaves, neqRangeSiblingLeaves, BuildSource, BuildSourceBounded`, `internal/vm/rangenode.go:RangeNode.NeqPairedRange`, `internal/vm/traceql_compiler.go:extractNeqNode, extractNeqNumericNode`. Tests: `neq_unscoped_collision_realvi_test.go`, `structural_oracle_comparison_test.go` Shape7, full `value_index_oracle_comparison_test.go` suite, `neq_metrics_realvi_test.go`. Issue #213.
...
## NOTE-VI-092 — ExecuteStructuralFromIndex: decoupling "walk anchor" from "discovery seed" for 2-node positive structural queries (issue #489, plan-d.md §D4)

**What this is.** `ExecuteStructuralFromIndex(ctx, q, leftSource, rightSource, isSelective, traceGroupStore, tenant, indexPrefix, readerFor, minTS, maxTS, indexOnly, opts) (*StructuralResult, bool, error)` (`internal/modules/executor/structural_index.go`) answers a 2-node POSITIVE structural query (`>>`, `>`, `~`, `<<`, `<`) using the value index for candidate-trace discovery and TraceGroup materialization for whole-trace ancestor/sibling resolution, with no full-block scan. Mirrors `QueryTraceQLFromIndex`'s `(results, ok, err)` decline contract so tempo's Fetch dispatch (DT1) can try it before falling back to `ExecuteStructural`.

**Central design correction: "lead/confirm" doesn't compose the way the original plan assumed.** plan-d.md's original design ("pick whichever side is cheaper as lead, confirm the other side via D3B's `verifyCandidateSpans`") only composes correctly when the non-lead side is the operator's terminal/output side (R, per `applyStructuralOp`'s hardwired node1-is-output convention) — confirming L would require checking a candidate's ANCESTOR/SIBLING spans, not the candidate span's own row, which `verifyCandidateSpans` (a per-row predicate check) cannot do. Team-lead checkpoint ruling (2026-07-07) replaced the single "lead" concept with two DECOUPLED concepts:
- **Walk anchor (always L):** L's exact VI matches feed `nodeMatch` bit0 for every candidate trace's structural walk. R is treated as provisionally match-all (bit1 always set, mirroring `compileStructuralPair`'s own nil-filter-matches-all convention, NOTE-VI-087) so the walk finds every structurally-valid R-candidate; D3B then confirms R's REAL filter against exactly those walk survivors — this is what `verifyCandidateSpans` was built for.
- **Discovery seed (either side, chosen by cost):** which side's VI exact-match TraceID set drives the INITIAL candidate-trace list, before any TraceGroup lookups. A pure efficiency choice — a trace with no L match AND no R match anywhere cannot be a true answer for any of these operators, so seeding from either side's exact-match set is sound and non-lossy. Seed rule: seed from R only when `isSelective` classifies R as Selective and L is NOT; L remains the zero-extra-query default on a tie or unknown classification, since L is already resolved as the walk anchor regardless of seed choice.

**Invariant: bit-role occupancy never swaps, regardless of which side seeds discovery.** `nodeMatch` bit0 is ALWAYS the walk-anchor (L)'s real VI-match bit; bit1 is ALWAYS R's bit (provisional-match-all pre-D3B-confirmation, then real post-confirmation). This role assignment is fixed by `resolvedSpansToStructuralRecs` unconditionally, independent of `chooseCandidateTraceIDs`' seed choice — seeding discovery from R's exact matches (when R is Selective and L is not) changes WHICH traces are even considered, never WHICH bit represents which side.

**Adversarial verification, not just designed.** validator-d2's adversarial check confirmed all three structural-operator edge cases hold under this split (including `~`'s self-exclusion case) — the provisional-R-then-confirm walk is algebraically identical regardless of which side seeded discovery, since bit1 is only ever read as a candidate gate during the walk, never as a source of truth for the final answer.

**Step-4 intersection prefilter is complementary, not a substitute for D3B.** When BOTH sides classify Selective, R is also VI-resolved and its exact-match TraceID set intersects the discovery seed before any TraceGroup lookup happens — a pure cost optimization (fewer TraceGroup lookups), independent of which side seeded discovery. `chooseCandidateTraceIDs` memoizes R's VI resolution so it is computed AT MOST ONCE regardless of whether the seed-choice branch or the intersection-prefilter branch (or both) need it — never two independently-computed, possibly-drifting resolutions.

**`StructuralSelectivityClassifier` is a forced dependency-injection, not a design preference.** `isSelective StructuralSelectivityClassifier` (a `func(prog *vm.Program) bool`) is injected because `executor` cannot import `internal/modules/queryplan` directly: `queryplan` imports `vibuilder` (`indexable.go`), which itself imports `executor` (`builder.go`) — a genuine Go import cycle, confirmed by attempting the build, not assumed from reading import graphs. This applies the SAME injection pattern D3's own root-vs-executor ruling already established for this class of problem ("only if root-inseparable dependencies exist, use injection: executor defines a Func type, root supplies the implementation") — root's own D5 wrapper safely imports `queryplan` today and closes over `queryplan.ClassifyProgramVCNT(...) == queryplan.Selective` to build the classifier it passes in. The nil-guard on this classifier's construction (`len(vcntData) > 0 && rightSource != nil`, root `structural.go`) independently avoids the same `rightSource.AllResults()` nil-panic risk that task #10 fixed one layer down inside this function — belt-and-suspenders, not a coincidence.

**Join key: `structuralSpanAddr{sourceRef, blockPage, rowIdx}`, NEVER `SpanID` (task #12, FIX-D4-CRITICAL — root cause verified, not merely patched).** `VILookupResult.SpanID` cannot be used to join a value-index match against a `TraceGroup`'s `SpanEntry`: the on-disk `BucketGroup` format's `SpanRef` struct (`valueindex/bucketfile.go`) has NO SpanID field at all — only `SpanIndexes []uint16` and `TraceID [16]byte`. `assembleBucket` (`valueindex/writer.go:313`) unconditionally constructs `SpanRef{TraceID: re.traceID}`, discarding whatever SpanID the extraction step attached, for every entry `WriteValueIndexL0` ever writes. This is a STRUCTURAL property of the wire format (which was designed around block-identity + row-index addressing, per NOTE-VI-045, not SpanID) — not a conditional keyed on which `AddEntry*` variant was used upstream. `groupVILookupResultsByTrace` keys every VI-derived join by `structuralSpanAddr{sourceRef, blockPage, rowIdx}` instead — the identity every `WriteValueIndexL0`-written entry actually and unconditionally guarantees, matching `TraceGroup.Spans[]`'s own `SourceRef`+`BlockRef.PageNum`+`RowIdx` addressing exactly (the same address D3's `ResolvedSpan` already uses). Caught by, not designed against, a real-write-path test: every fixture in this phase's original test suite hand-constructed its `VILookupResult` values with an artificially-correct SpanID, so the defect was invisible until coder-d3's first end-to-end test exercised the genuine `WriteValueIndexL0` → search-VI → `QueryStructuralFromIndex` round trip. See NOTE-VI-094 for the generalized invariant this incident established, and TEST-VI-22/EX-36 for the resulting mandatory test class.

Back-refs: `internal/modules/executor/structural_index.go:ExecuteStructuralFromIndex, chooseCandidateTraceIDs, chooseDiscoverySeed, resolvedSpansToStructuralRecs, evalOneStructuralCandidateTrace, materializeConfirmedSpanBlocks, StructuralSelectivityClassifier, structuralSpanAddr, groupVILookupResultsByTrace`, `internal/modules/valueindex/bucketfile.go:SpanRef`, `internal/modules/valueindex/writer.go:assembleBucket`. Tests: `structural_index_test.go`, `structural_index_golden_test.go`, `structural_index_seed_test.go`, `structural_index_realvi_test.go`. Issue #489, tasks #10, #12.

---

## NOTE-VI-093 — ExecuteNegatedStructuralFromIndex: architecturally distinct from D4, not a parameter flip — right-exclusive discovery and an unconditional, trace-size-proportional cost model (issue #489, plan-d.md §D6)

**What this is.** `ExecuteNegatedStructuralFromIndex(ctx, q, rightSource, traceGroupStore, tenant, indexPrefix, readerFor, minTS, maxTS, indexOnly, opts) (*StructuralResult, bool, error)` (`internal/modules/executor/structural_index_negated.go`) answers a 2-node NEGATED structural query (`!>>`, `!>`, `!~`) using the value index for RIGHT-side candidate-trace discovery only, and TraceGroup materialization for whole-trace resolution. Mirrors `ExecuteStructuralFromIndex`'s (D4, NOTE-VI-092) `(result, ok, err)` decline contract.

**Invariant: the negated (left) side is structurally unresolvable via VI, enforced by the function's own signature, not by a runtime convention a caller could violate.** `ExecuteNegatedStructuralFromIndex` has NO `leftSource` parameter at all — contrast `ExecuteStructuralFromIndex` (NOTE-VI-092), which takes both `leftSource` and `rightSource`. There is no way for any caller, by mistake or otherwise, to wire a left-side `ValueIndexSource` into this function; candidate-trace discovery is driven EXCLUSIVELY by `rightSource`.

**Why this is a separate engine, not D4 with parameters swapped (brainstorm §3 audit).** D4's candidate-trace set is anchored by L's exact VI matches (the walk anchor is always L). D6 cannot reuse that shape: a trace with ZERO left-filter matches trivially satisfies `!>>`/`!>`/`!~` for every right-match span (vacuous truth) — the negated (left) side therefore cannot be scored or led via VI the way D4's positive path assumes, because the value index has no fast negation path at all (the same documented limitation as negated FILTER predicates, SPEC-ROOT-019).

**Confirmation is unconditional, not selectivity-gated — this is the load-bearing asymmetry to call out explicitly.** The left side is confirmed ONLY after the whole candidate trace's TraceGroup tree is fetched, via D3B's `verifyCandidateSpans` (NOTE-VI-090) applied UNCONDITIONALLY to every span in the tree — never a selectivity-narrowed subset the way D4 conditionally applies the same primitive. This makes D6's per-candidate cost strictly higher than D4's and of a DIFFERENT shape: proportional to trace size (a full-tree confirmation every time), not to the negated side's own VI selectivity, because no such selectivity signal exists for the negated side. Callers dispatching between D4 and D6 must not assume a shared cost model.

**Both structuralSpanRec bits are real ground truth here, unlike D4's provisional-R-then-confirm convention.** bit0 (`nodeMatch&0x01`) is set only for spans D3B's unconditional confirmation actually matches against the real left filter; bit1 (`nodeMatch&0x02`) is set only for spans the right VI source exactly matched. `applyStructuralOp`'s existing `evalOpNotDescendantStruct`/`evalOpNotChildStruct`/`evalOpNotSiblingStruct` (`stream_structural.go`) are reused completely unchanged — the same parity-critical reuse D1's golden table exists to verify.

**D6's mandated audit (task #12, item 2) is confirmed complete, with its predicted outcome verified correct.** The right (tested) side's marking (`rightMatchAddrs`) had the IDENTICAL `structuralSpanAddr`-not-`SpanID` defect D4 had (both derive from raw `VILookupResult` values) and is fixed the same way (NOTE-VI-092). The left (negated) side's marking (`leftMatchSpanIDs`, sourced from D3B's `verifyCandidateSpans` output) is CONFIRMED SAFE to key by SpanID exactly as originally written — both operands of that specific comparison are the SAME `resolvedSpans` slice (real, per-row `TraceGroup`/`ResolvedSpan` data), never a raw `VILookupResult`, so the wire-format limitation NOTE-VI-092/094 describe never applies to it.

**Ruling 5 (Partial TraceGroup is a coverage gap) applies identically here — see SPEC-STRUCT-11 for the shared contract, not duplicated in this note.** The justification is the mirror image of D4's: a partial tree risks false NEGATIVES for D4's positive operators, but false POSITIVES for D6's negated operators (an orphan span whose true left-matching ancestor/parent/sibling lives outside the assembled partial data would wrongly appear to satisfy a negation). Both directions converge on the same rule: never treat a narrower-than-true assembled tree as an authoritatively complete answer.

Back-refs: `internal/modules/executor/structural_index_negated.go:ExecuteNegatedStructuralFromIndex, evalOneNegatedStructuralCandidateTrace, resolvedSpansToNegatedStructuralRecs, structuralSpanAddr` usage in `rightMatchAddrs`. Tests: `structural_index_negated_test.go`, `structural_index_negated_realvi_test.go`. Issue #489, task #12.

---

## NOTE-VI-095 — Phase D holistic-review fix pass (2026-07-08): errgroup panic isolation, D6 root reachability, nil-source guards, and per-reader I/O coalescing (issue #489)

**Context.** `.bob/state/review.md` and `.bob/state/go-presubmit.md` (both 2026-07-08) independently found the same CRITICAL/HIGH cluster in the just-landed Phase D changeset (D3/D3B/D4/D5/D6). This note records the fixes as applied; it does not change any SPEC-STRUCT-9/10/11 contract or SPEC-ROOT-001/015 invariant — it makes existing invariants actually hold at every point of use, per those reviews' own framing ("guards/plumbing/perf", not a design change).

**1. `MaterializeTraceGroupMultiFile`'s `errgroup.Go` closures now recover panics (CRITICAL, SPEC-ROOT-001).** `readerFor` is a caller-supplied callback (`StructuralReaderProvider`) this package does not control; a panic inside it (or inside `ResolveTraceGroupSourceRef`) previously crashed the whole host process, not just the one in-flight query. Fixed by copying this repo's own established pattern (`internal/modules/valueindexcompactor/service.go`'s `g.Go` closure, itself tagged `// SPEC-ROOT-001`), adapted to RETURN the recovered panic as an error (not swallow it as the compactor's fire-and-forget case does) since `g.Wait()`'s caller — a live query — needs to see the failure. Also added a `reader == nil` guard immediately after `readerFor` returns, treating a well-behaved-by-convention-but-not-type-enforced `(nil, nil)` return the same as an error (`ErrStructuralMultiFileCoverageGap`). Tests: `structural_multifile_test.go`'s `TestMaterializeTraceGroupMultiFile_ReaderForPanics_ReturnsErrorNotCrash` (asserts a returned error, not a crashed test binary, under `-race`) and `_ReaderForReturnsNilReader_ReturnsTypedCoverageGapError`.

**2. D6 now has a root-package public entry point: `QueryNegatedStructuralFromIndex` (`structural.go`), pre-authorized public API addition.** Before this fix, `ExecuteNegatedStructuralFromIndex` (NOTE-VI-093) was reachable only from tests inside `internal/modules/executor` — Go's `internal/` visibility rule made it a permanent, unreachable dead end for any external caller (Tempo included), despite `SPEC-ROOT-020`'s own text describing routing to it as though it already existed at the root layer. `QueryNegatedStructuralFromIndex` mirrors `QueryStructuralFromIndex`'s shape/decline/recover conventions exactly, with the parameter set narrowed to match D6's engine (single `rightSource`, no `leftSource`/`vcntData`/`vcntDir`/selectivity classifier — D6 has no cost-based intersection-prefilter equivalent to D4's step 4). `structural.go`'s `QueryStructuralFromIndex` doc comment (previously stale: "routed to a separate D6 function, not yet implemented") now correctly names `QueryNegatedStructuralFromIndex` as the negated-operator destination. Test: `structural_index_realvi_test.go`'s `TestQueryNegatedStructuralFromIndex_RealWriteValueIndexL0_EndToEnd` — a genuinely non-vacuous real-write-path test (two `svc-leaf` spans in the same trace, one WITH a `svc-root` ancestor, one WITHOUT; only the ancestor-free one survives `!>>`), calling the new root wrapper itself end to end through `WriteValueIndexL0` → `BuildValueIndexSource` → `QueryNegatedStructuralFromIndex` — unlike the pre-existing `TestExecuteNegatedStructuralFromIndex_RealWriteValueIndexL0_RightSideSpanIDIsZero` (kept unchanged; it still pins a real, distinct fact about `VILookupResult.SpanID`), this new test is the first to prove the engine correct AND reachable end to end through the public API.

**3. Nil-source guards added at the point of use inside both index-driven engines (HIGH, SPEC-ROOT-001).** `viMatchSpans` (`metrics_trace.go`) calls `source.AllResults()` unconditionally whenever `prog` is nil (the match-all convention, NOTE-VI-087) with no nil-check of `source` — calling a method on a nil interface value panics unconditionally in Go. Previously this was guarded ONLY by the root wrapper (`structural.go`'s `leftSource == nil` check), one layer above `ExecuteStructuralFromIndex` itself, and not guarded AT ALL for `ExecuteNegatedStructuralFromIndex` (no root wrapper existed at all — see item 2 above). Fixed: `ExecuteStructuralFromIndex` now checks `leftSource == nil` (the walk anchor, REQUIRED) immediately after the negation-op check; `ExecuteNegatedStructuralFromIndex` now checks `rightSource == nil` (D6's only VI-resolved side, REQUIRED) immediately after the positive-op check. Both return the routine `(nil, false, nil)` decline, consistent with the "no index coverage supplied" family already used by the `!leftOK`/`!rightOK` branches immediately below each guard. D4's `rightSource` (the optional side) is deliberately NOT guarded here — `chooseCandidateTraceIDs`' own `rightSource != nil` check (task #10, NOTE-VI-092) already makes that side's nil-safety unconditional. Tests: `structural_index_test.go`'s `TestExecuteStructuralFromIndex_NilLeftSourceWithMatchAllLeftLeg_DeclinesNoPanic`, `structural_index_negated_test.go`'s `TestExecuteNegatedStructuralFromIndex_NilRightSourceWithMatchAllRightLeg_DeclinesNoPanic`.

**4. Per-block `ReadBlocks` calls batched per reader — `materializeConfirmedSpanBlocks` and `verifyCandidateSpans`/`evaluateProgramAgainstBlock` (HIGH, `io_ops` regression).** Both previously issued `reader.ReadBlocks([]int{blockIdx})` — a single-element slice — once per distinct block inside a loop, discarding `modules_reader.Reader`'s adjacent-block coalescing (`reader.go`: "Adjacent block ranges are merged into as few I/O operations as possible"). Fixed to mirror the sibling primitives built in this same phase (`ResolveTraceGroupSourceRef`, NOTE-VI-089, and `search_trace_vi.go`'s own coalesced multi-block fetch): group needed block indices per `*modules_reader.Reader` first, issue exactly ONE `ReadBlocks` call per reader, THEN parse/evaluate each block from the already-fetched raw bytes. `evaluateProgramAgainstBlock`'s signature changed to accept the already-fetched `raw []byte` instead of doing its own single-block `ReadBlocks` call (its parse/`ColumnPredicate` logic is otherwise unchanged; all existing error-wrapping text preserved). Tests (new, white-box, proving the coalescing by counting the underlying provider's `ReadAt` calls, not just asserting correctness): `structural_verify_internal_test.go`'s `TestVerifyCandidateSpans_MultiBlockSameReader_CoalescesIntoOneReadBlocksCall`; `structural_index_io_internal_test.go`'s `TestMaterializeConfirmedSpanBlocks_MultiBlockSameReader_CoalescesIntoOneReadBlocksCall` and `_TwoReaders_OneReadBlocksCallPerReader` (proving the batching is per-reader, not globally collapsed).

**5. `evalOneStructuralCandidateTrace`'s dead `indexOnly` parameter removed (MEDIUM, cross-task consistency).** D4's per-candidate helper carried an `indexOnly bool` parameter that was never read for control flow (`_ = indexOnly // reserved: ...`); D6's mirror (`evalOneNegatedStructuralCandidateTrace`) never had the parameter at all. Removed from D4's signature and its one call site for symmetry — pure signature cleanup, no behavior change (SPEC-STRUCT-11's "`Partial` is an unconditional coverage-gap error regardless of `indexOnly`" rule was never actually implemented via this parameter in either engine).

**6. `ErrStructuralIndexCoverageGap`'s `Partial`-TraceGroup return sites now wrap with the trace ID (MEDIUM, observability).** Both D4 (`structural_index.go`) and D6 (`structural_index_negated.go`) now return `fmt.Errorf("trace %x: %w", traceID, ErrStructuralIndexCoverageGap)` instead of the bare sentinel — `errors.Is` compatibility (DT2's contract) is unaffected by `%w` wrapping. Only the `Partial`-tree sites were changed; the earlier `indexOnly` coverage-gap branches in both engines were left bare per the review's own scoping (not flagged there).

**7. Double-decode of confirmed spans' blocks (MEDIUM, performance) — explicitly SKIPPED, not fixed.** D3B's `evaluateProgramAgainstBlock` (restricted-columns parse, for confirmation) and `materializeConfirmedSpanBlocks` (`WantAll()` parse, for final materialization) still parse the same block twice for every block containing a confirmed match. A clean fix would require `verifyCandidateSpans` to return the already-parsed `*modules_reader.Block` (or raw bytes) alongside each confirmed span so `materializeConfirmedSpanBlocks` can skip re-parsing — but `verifyCandidateSpans` parses with `WantOnly(prog's columns)` while materialization needs `WantAll()`, so sharing the parse would mean confirmation always decodes every column (a real cost increase for the — usually larger — candidate set that FAILS confirmation), not a pure refactor. This crosses the fix-prompt's stated boundary ("only fix if a bounded, clean fix exists") and risks destabilizing both D4 and D6's call sites for a MEDIUM, explicitly-deferrable finding (the review itself: "not blocking — flagged for a follow-up, not this phase"). Left as a follow-up.

**8. Two stale comments corrected (LOW).** `structural_traceresolve.go`'s `rowMatchesTraceIDColumn` doc comment no longer claims a root-side `rowMatchesTraceID` twin exists (it was removed when `materializeTraceGroup` became a thin delegate — see NOTE-VI-089). `internal/modules/valueindex/query.go`'s `LookupResult`/`SpanID` comments corrected per NOTE-VI-094's own recommendation — see that note's 2026-07-08 amendment in `internal/modules/valueindex/NOTES.md`.
