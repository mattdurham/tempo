...
func lookupIntrinsicFields(
	...
	// map for any requested identity column the loop above did not populate. This serves
	// SpanMatchFromRow's match-all / block-scan materialization on omit-identity files.
	if err := fillIdentityFromSpanTreeIntoMaps(r, selected, wantCols, result); err != nil {
		return nil, err
	}
...
// fillIdentityFromSpanTreeIntoMaps populates trace:id/span:id/span:parent_id ([]byte values)
// into the per-ref result maps from each ref's block SpanTree identity map, for any of the
// three columns that is requested (wantCols==nil means all) but absent from the IntrinsicTOC.
// Maps are fetched once per distinct block referenced. NOTE-476 (issue #394).
func fillIdentityFromSpanTreeIntoMaps(
	r *modules_reader.Reader,
	selected []modules_shared.BlockRef,
	wantCols map[string]struct{},
	...
	for i, ref := range selected {
		if !haveMap || ref.BlockIdx != curBlock {
			...
			if err != nil {
				return fmt.Errorf("lookupIntrinsicFields: SpanTreeIdentityForBlock: %w", err)
			}
			curMap = m
		...
		if wantTrace {
			...
			result[i][colNameTraceID] = tid
		}
		if wantSpan {
			sid := make([]byte, 8)
			copy(sid, rec.SpanID[:])
			result[i][colNameSpanID] = sid
		}
		if wantParent && rec.ParentID != ([8]byte{}) {
			pid := make([]byte, 8)
			copy(pid, rec.ParentID[:])
			result[i][colNameParentID] = pid
		}
	}
	return nil
}
