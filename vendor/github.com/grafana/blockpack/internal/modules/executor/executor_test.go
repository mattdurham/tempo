...
// PROJ-01: SelectColumns restricts eager column decode for no-predicate queries.
// For a "{}" query, ProgramWantColumns returns nil (decode all). When SelectColumns
// is set, only the requested columns and trace intrinsics are eagerly decoded —
// other span attribute columns are registered lazily (not yet decoded).
func TestCollect_SelectColumns_ProjectsColumns(t *testing.T) {
	var buf bytes.Buffer
	w := mustNewWriter(t, &buf, 0)
	tid := [16]byte{0xC1}
	for i := range 4 {
		addSpan(t, w, tid, i, "op", tracev1.Span_SPAN_KIND_SERVER,
			[]*commonv1.KeyValue{
				...
				strAttr("env", "prod"), // not in SelectColumns — should remain lazy
			},
			map[string]any{"service.name": "svc"})
	}
	mustFlush(t, w)
	...
	rows, _, err := executor.Collect(r, prog, executor.CollectOptions{
		SelectColumns: []string{"span.http.method"},
	})
	require.NoError(t, err)
	require.Equal(t, 4, len(rows), "all spans must match {}")

	for _, row := range rows {
		require.NotNil(t, row.Block, "row must have a block")
		// Projected column must be eagerly decoded.
		col := row.Block.GetColumn("span.http.method")
		require.NotNil(t, col, "projected column span.http.method must be registered")
		assert.True(t, col.IsDecoded(),
			"projected column span.http.method must be eagerly decoded (not lazy)")
		// Non-projected attribute must be registered but remain lazy (not eagerly decoded).
		// Column objects still exist for lazy access; only readColumnEncoding is deferred.
		envCol := row.Block.GetColumn("span.env")
		require.NotNil(t, envCol, "span.env must be registered (lazy)")
		assert.False(t, envCol.IsDecoded(),
			"non-projected column span.env must remain lazy (not eagerly decoded)")
	}
}

// PROJ-02: SelectColumns baseline — without SelectColumns, all columns are eagerly decoded.
func TestCollect_SelectColumns_AllColumnsWhenEmpty(t *testing.T) {
	var buf bytes.Buffer
	w := mustNewWriter(t, &buf, 0)
	tid := [16]byte{0xC2}
	for i := range 2 {
	...
	for _, row := range rows {
		require.NotNil(t, row.Block)
		httpCol := row.Block.GetColumn("span.http.method")
		require.NotNil(t, httpCol, "without SelectColumns, span.http.method must be present")
		assert.True(t, httpCol.IsDecoded(),
			"without SelectColumns, span.http.method must be eagerly decoded")
		envCol := row.Block.GetColumn("span.env")
		require.NotNil(t, envCol, "without SelectColumns, span.env must be present")
		assert.True(t, envCol.IsDecoded(),
			"without SelectColumns, span.env must be eagerly decoded")
	}
}

// PROJ-03: SelectColumns with a predicate — predicate columns are always decoded even
// if not in SelectColumns, ensuring filter correctness.
func TestCollect_SelectColumns_IncludesPredicateColumns(t *testing.T) {
	var buf bytes.Buffer
	w := mustNewWriter(t, &buf, 0)
	tid := [16]byte{0xC3}
	addSpan(t, w, tid, 0, "op", tracev1.Span_SPAN_KIND_SERVER,
		[]*commonv1.KeyValue{strAttr("http.method", "GET"), strAttr("env", "prod")},
		map[string]any{"service.name": "svc"})
	addSpan(t, w, tid, 1, "op", tracev1.Span_SPAN_KIND_SERVER,
		[]*commonv1.KeyValue{strAttr("http.method", "DELETE"), strAttr("env", "prod")},
		map[string]any{"service.name": "svc"})
	mustFlush(t, w)
	r := openReader(t, buf.Bytes())

	// Predicate on span.http.method; SelectColumns asks only for span.env.
	// The predicate column (span.http.method) must be decoded for filtering to work.
	prog := compileQuery(t, `{ span.http.method = "GET" }`)
	rows, _, err := executor.Collect(r, prog, executor.CollectOptions{
		SelectColumns: []string{"span.env"},
	})
	require.NoError(t, err)
	require.Equal(t, 1, len(rows), "only GET span matches")

	// The predicate column must be present and decoded even though it is not in SelectColumns.
	require.NotNil(t, rows[0].Block, "row must have a block")
	predCol := rows[0].Block.GetColumn("span.http.method")
	require.NotNil(t, predCol, "predicate column span.http.method must be registered")
	assert.True(t, predCol.IsDecoded(),
		"predicate column span.http.method must be eagerly decoded for filter correctness")
}

// PROJ-04: AllColumns=true takes precedence over SelectColumns — SelectColumns is silently
// ignored when AllColumns=true; all columns are decoded as if SelectColumns were empty.
func TestCollect_SelectColumnsIgnoredWhenAllColumns(t *testing.T) {
	var buf bytes.Buffer
	w := mustNewWriter(t, &buf, 0)
	tid := [16]byte{0xC4}
	for i := range 2 {
	...
	// AllColumns=true with a non-empty SelectColumns — SelectColumns must have no effect.
	rowsBoth, _, err := executor.Collect(r, prog, executor.CollectOptions{
		AllColumns:    true,
		SelectColumns: []string{"span.http.method"}, // should be ignored
	})
	require.NoError(t, err)

	// AllColumns=true without SelectColumns — baseline.
	rowsAllOnly, _, err := executor.Collect(r, prog, executor.CollectOptions{
		AllColumns: true,
	})
	require.NoError(t, err)

	require.Equal(t, len(rowsAllOnly), len(rowsBoth),
		"AllColumns=true with SelectColumns must return same row count as AllColumns=true alone")

	// With AllColumns=true, non-projected columns must be eagerly decoded in both cases.
	for i, row := range rowsBoth {
		require.NotNil(t, row.Block, "row %d must have a block", i)
		envCol := row.Block.GetColumn("span.env")
		require.NotNil(t, envCol, "AllColumns=true: span.env must be registered (row %d)", i)
		assert.True(t, envCol.IsDecoded(),
			"AllColumns=true: span.env must be eagerly decoded even when SelectColumns is set (row %d)", i)
	}
}

// PROJ-05: SelectColumns with predicate columns — output-only columns are deferred to the
// second parse pass so they are only decoded for matching blocks, not every scanned block.
// This verifies that the deferred-decode path produces correct results: SelectColumns appear
// on matched rows even though they were not in the first-pass wantColumns.
func TestCollect_SelectColumns_DeferredToSecondPass(t *testing.T) {
	var buf bytes.Buffer
	w := mustNewWriter(t, &buf, 0)
	tid := [16]byte{0xC5}
	// Two spans: one matches the predicate (GET), one does not (DELETE).
	// Both have span.env; only the GET span should appear in results.
	addSpan(t, w, tid, 0, "match-op", tracev1.Span_SPAN_KIND_SERVER,
		[]*commonv1.KeyValue{strAttr("http.method", "GET"), strAttr("env", "prod")},
		map[string]any{"service.name": "svc"})
	addSpan(t, w, tid, 1, "no-match-op", tracev1.Span_SPAN_KIND_SERVER,
		[]*commonv1.KeyValue{strAttr("http.method", "DELETE"), strAttr("env", "staging")},
		map[string]any{"service.name": "svc"})
	mustFlush(t, w)
	r := openReader(t, buf.Bytes())

	// Predicate on span.http.method; SelectColumns asks for span.env (output-only column).
	// span.env is NOT a predicate column — it is deferred to secondPassCols and decoded
	// only for the matching block.
	prog := compileQuery(t, `{ span.http.method = "GET" }`)
	rows, _, err := executor.Collect(r, prog, executor.CollectOptions{
		SelectColumns: []string{"span.env"},
	})
	...
	// span.env must be decoded on the matched row (decoded via second pass).
	require.NotNil(t, rows[0].Block, "matched row must have a block")
	envCol := rows[0].Block.GetColumn("span.env")
	require.NotNil(t, envCol, "span.env must be registered on matched row")
	assert.True(t, envCol.IsDecoded(),
		"span.env (SelectColumns, output-only) must be decoded on matched row")
}
