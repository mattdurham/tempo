...
**Assertions:** `len(rows) == 0`. Confirms both predicates return 0 for absent columns
and their AND intersection is also 0.

Back-ref: `internal/modules/executor/executor_test.go:TestANDPredicateNotEqual_AbsentColumns_NoFalsePositive`
...
**Assertions:** `len(rows) == 0`. Absent column returns empty RowSet, not FullScan.

Back-ref: `internal/modules/executor/executor_test.go:TestScanNotEqual_AbsentColumn_ReturnsZero`
