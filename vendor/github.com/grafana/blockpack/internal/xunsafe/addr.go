...
import (
	...
	"github.com/grafana/blockpack/internal/xunsafe/layout"
)
...
func (a Addr[T]) RoundUpTo(align int) Addr[T] {
	return Addr[T](layout.RoundUp(uintptr(a), uintptr(align))) //nolint:gosec // G115: uintptr conversions are intentional in this unsafe package
}
...
func (a Addr[T]) Format(state fmt.State, verb rune) {
	if verb == 'v' {
		_, _ = fmt.Fprintf(state, "%#x", uintptr(a)) //nolint:gosec // G115: intentional uintptr for formatting
		return
	}

	_, _ = fmt.Fprintf(state, fmt.FormatString(state, verb), uintptr(a)) //nolint:gosec // G115: intentional uintptr for formatting
}
