...
func Size[T any]() int {
	var z T
	return int(unsafe.Sizeof(z)) //nolint:gosec // G115: uintptr→int is safe; sizes fit in int on all supported platforms
}
...
func Align[T any]() int {
	var z T
	return int(unsafe.Alignof(z)) //nolint:gosec // G115: uintptr→int is safe; alignments fit in int on all supported platforms
}
