...
func run(path string) error {
	...
	// ── File-level TOC ────────────────────────────────────────────────────
	// NOTE-422: only block index, TS index, and value-index sections can appear in a
	// v2 lean file's ToC. Bloom/trace/trace-chunked sections were removed at the v2
	// cutover and the reader rejects any pre-v2 file, so those labels were dropped.
