...
// sub-benchmark scale label, e.g. "small"
// "blockpack" or "parquet"
...
// parseOTELDemoBenchType extracts the benchType, queryName, and format fields from a
// benchmark full name. Returns ("", "", "") when the name does not match any known suite.
func parseOTELDemoBenchType(fullName string) (benchType, queryName, format string) {
	if strings.Contains(fullName, "RealWorldQueries/") {
		benchType = "realworld"
		...
		if idx := strings.Index(fullName, prefix); idx >= 0 {
		...
		return benchType, queryName, format
	}
	if strings.Contains(fullName, "ComprehensiveSuite/") {
		benchType = "comprehensive"
		if idx := strings.Index(fullName, "ComprehensiveSuite/"); idx >= 0 {
	...
	if strings.Contains(fullName, "GetByTraceID/") {
		benchType = "trace"
		queryName = "trace-by-id"
		if idx := strings.Index(fullName, "GetByTraceID/"); idx >= 0 {
...
// applyOTELDemoMetric populates one metric field of od from a single benchmark value.
func applyOTELDemoMetric(od *OTELDemoResult, unit string, value float64) {
	switch unit {
	case metricSecOp:
		od.TimeNs = int64(value * 1_000_000_000)
		od.TimeMs = value * 1000
	case metricBytesOp:
		od.MemoryBytes = int64(value)
		od.MemoryMB = value / (1024 * 1024)
	case metricAllocsOp:
		od.Allocs = int64(value)
	case "blocks":
		od.BlocksScanned = int64(value)
	case metricBytesRead:
		od.BytesRead = int64(value)
		od.BytesReadMB = value / (1024 * 1024)
	case metricIOOps:
		od.IOOps = int64(value)
	case metricCPUMs:
		od.CPUMs = value
	case "traces":
		od.Traces = int64(value)
	case "spans":
		od.Spans = int64(value)
	case "rows":
		if od.Spans == 0 {
			od.Spans = int64(value) // legacy: some benchmarks report "rows" instead of "spans"
		}
	...
	case metricCostLambdaCompute:
		od.CostLambdaCompute = value
	case metricCostLambdaRequests:
		od.CostLambdaRequests = value
...
func parseOTELDemoResult(result *benchfmt.Result, fullName string) *OTELDemoResult {
	benchType, queryName, format := parseOTELDemoBenchType(fullName)
	if benchType == "" {
		return nil
	}
	...
	for _, v := range result.Values {
		applyOTELDemoMetric(od, v.Unit, v.Value)
	}
	return od
}
...
func formatWinCell(blockpackWins, bothExist bool, cScore, pScore float64) string {
	if !bothExist {
		return winCellTie
	}
	if blockpackWins {
...
func formatFileSizeBar(blockpackMB, parquetMB float64) string {
	...
	if blockpackMB > 0 && parquetMB > 0 {
		...
		if ratio >= 1 {
			ratioText = fmt.Sprintf(
				`<span class="file-size-ratio">Parquet is <strong>%.1fx larger</strong></span>`,
				ratio,
			)
		} else {
...
func computeOTELDemoRowMetrics(comp OTELDemoComparison) otelDemoRowMetrics {
	...
	m.parquetTime = formatWithSpeedup(
		formatTime(m.pVal.TimeMs),
		safeSpeedup(m.cVal.TimeMs, m.pVal.TimeMs),
		!m.blockpackWins,
	)

	m.blockpackCPU = formatWithSpeedup(formatTime(m.cVal.CPUMs), cpuSpeedup, m.cVal.CPUMs < m.pVal.CPUMs)
	m.parquetCPU = formatWithSpeedup(
		formatTime(m.pVal.CPUMs),
		safeSpeedup(m.cVal.CPUMs, m.pVal.CPUMs),
		m.pVal.CPUMs < m.cVal.CPUMs,
	)

	m.blockpackBytes = formatWithSpeedup(
		formatMemory(m.cVal.BytesReadMB),
		bytesSpeedup,
		m.cVal.BytesRead < m.pVal.BytesRead,
	)
	m.parquetBytes = formatWithSpeedup(
		formatMemory(m.pVal.BytesReadMB),
		safeSpeedup(float64(m.cVal.BytesRead), float64(m.pVal.BytesRead)),
		m.pVal.BytesRead < m.cVal.BytesRead,
	)

	m.blockpackIOOps = formatWithSpeedup(
		addCommas(fmt.Sprintf("%d", m.cVal.IOOps)),
		ioOpsSpeedup,
		m.cVal.IOOps < m.pVal.IOOps,
	)
	m.parquetIOOps = formatWithSpeedup(
		addCommas(fmt.Sprintf("%d", m.pVal.IOOps)),
		safeSpeedup(float64(m.cVal.IOOps), float64(m.pVal.IOOps)),
		m.pVal.IOOps < m.cVal.IOOps,
	)

	m.blockpackMemory = formatWithSpeedup(
		formatMemory(m.cVal.MemoryMB),
		memorySpeedup,
		m.cVal.MemoryMB < m.pVal.MemoryMB,
	)
	m.parquetMemory = formatWithSpeedup(
		formatMemory(m.pVal.MemoryMB),
		safeSpeedup(m.cVal.MemoryMB, m.pVal.MemoryMB),
		m.pVal.MemoryMB < m.cVal.MemoryMB,
	)

	m.blockpackAllocs = formatWithSpeedup(
		addCommas(fmt.Sprintf("%d", m.cVal.Allocs)),
		allocsSpeedup,
		m.cVal.Allocs < m.pVal.Allocs,
	)
	m.parquetAllocs = formatWithSpeedup(
		addCommas(fmt.Sprintf("%d", m.pVal.Allocs)),
		safeSpeedup(float64(m.cVal.Allocs), float64(m.pVal.Allocs)),
		m.pVal.Allocs < m.cVal.Allocs,
	)

	m.blockpackCostHTML = formatCostWithTooltip(
		m.cVal.CostTotal,
		m.cVal.CostS3Get,
		m.cVal.CostS3Xfer,
		m.cVal.CostLambdaCompute,
		m.cVal.CostLambdaRequests,
	)
	m.parquetCostHTML = formatCostWithTooltip(
		m.pVal.CostTotal,
		m.pVal.CostS3Get,
		m.pVal.CostS3Xfer,
		m.pVal.CostLambdaCompute,
		m.pVal.CostLambdaRequests,
	)
...
`, html.EscapeString(p.QueryName), html.EscapeString(row.label))
			...
			r := row.r
			costHTML := formatCostWithTooltip(
				r.CostTotal,
				r.CostS3Get,
				r.CostS3Xfer,
				r.CostLambdaCompute,
				r.CostLambdaRequests,
			)
			fmt.Fprintf(
				&sb, `			<tr>
				<td class="query-name">%s</td>
...
func main() {
	...
	if len(otelDemo) == 0 && len(comparisons) == 0 && len(aggregations) == 0 && len(writePaths) == 0 &&
		len(traceMetrics) == 0 {
	...
	if *outputDir != "" {
		...
		metadata := BenchmarkMetadata{
			...
			GitHash:   getGitHash(),
			ResultsHash: generateResultsHash(
				fmt.Sprintf("%v%v%v%v%v", otelDemo, comparisons, aggregations, writePaths, traceMetrics),
			),
