...
// These helpers eliminate the need for callers to know column name strings and
// perform type assertions on the any values returned by Fields.GetField.
// =============================================================================
...
func SpanMatchesMetadata(spans []SpanMatch) (rootName, rootService string, startNano, durationNano uint64) {
	...
	for i := range spans {
		...
		if et, ok := m.EndNano(); ok && et > maxEnd {
			maxEnd = et
		}
